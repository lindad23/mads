...
def is_discrete_actions(env, adversary=False):
    if adversary:
        try:
            ...
            return False
        return action_space.__class__.__name__ == 'Discrete'
    else:
...
def _make_env(args):
    if args.env_name.startswith('CarRacing'):
        import envs.box2d  # noqa: F401
    elif args.env_name.startswith('BipedalWalker'):
        import envs.bipedalwalker  # noqa: F401
    elif args.env_name.startswith('MultiGrid'):
        import envs.multigrid  # noqa: F401
    ...
    if args.env_name.startswith('CarRacing'):
        ...
        if 'Adversarial' in args.env_name:
            env_kwargs.update({
                'use_categorical': args.use_categorical_adv,
                'use_sketch': args.use_sketch,
                'choose_start_pos': args.choose_start_pos,
            })
        if 'MADS' in args.env_name and hasattr(args, 'adversary_step_magnitude'):
            env_kwargs.update({
                'adversary_step_magnitude': args.adversary_step_magnitude,
            })
    elif args.env_name.startswith('BipedalWalker') and 'MADS' in args.env_name and hasattr(args, 'adversary_step_magnitude'):
        env_kwargs.update({
            'adversary_step_magnitude': args.adversary_step_magnitude,
        })
        if 'POET-Rose' in args.env_name and getattr(args, 'ued_algo', None) == 'domain_randomization':
            env_kwargs.update({
                'lock_params_on_reset': True,
            })
    elif args.env_name.startswith('MultiGrid') and getattr(args, 'use_mads_policy_feature', False):
        env_kwargs.update({
            'policy_feature_dim': getattr(args, 'mads_policy_feature_dim', 32 * 7),
        })
...
def create_parallel_env(args, adversary=True):
    ...
    is_car_racing_mads = is_car_racing and 'MADS' in args.env_name
    is_bipedalwalker = args.env_name.startswith('BipedalWalker')

    make_fn = lambda: _make_env(args)

    has_adversary_interface = not (
        is_car_racing and 'Adversarial' not in args.env_name and 'MADS' not in args.env_name
    )
    venv = ParallelAdversarialVecEnv(
        [make_fn]*args.num_processes,
        adversary=adversary and has_adversary_interface)
    ...
    if is_bipedalwalker or is_car_racing_mads:
        ued_venv = ParallelAdversarialVecEnv([make_fn]*args.num_processes, adversary=adversary)
        ued_venv = VecMonitor(venv=ued_venv, filename=None, keep_buf=100)
        ued_venv = VecNormalize(venv=ued_venv, ob=False, ret=args.normalize_returns)
        ued_venv = VecPreprocessImageWrapper(venv=ued_venv, obs_key=obs_key,
            transpose_order=transpose_order, scale=scale)
    elif is_multigrid:
        ued_venv = venv
