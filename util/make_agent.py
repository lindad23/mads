...
def model_for_car_racing_agent(
    ...
    if agent_type == 'adversary_env':
        adversary_observation_space = env.adversary_observation_space
        adversary_action_space = env.adversary_action_space
        adversary_spaces = getattr(
            adversary_observation_space,
            'spaces',
            adversary_observation_space)
        if 'policy_feature' in adversary_spaces:
...
def make_agent(name, env, args, device='cpu'):
    ...
    if args.algo == 'ppo':
        ...
        # Create storage
        derived_recurrent_arch = 'rnn'
        if hasattr(actor_critic, 'rnn') and actor_critic.rnn is not None and hasattr(actor_critic.rnn, 'arch'):
            derived_recurrent_arch = actor_critic.rnn.arch
        storage = RolloutStorage(
            ...
            recurrent_arch=derived_recurrent_arch,
