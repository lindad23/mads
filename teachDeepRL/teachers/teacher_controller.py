...
class TeacherController(object):
    def __init__(self, teacher, nb_test_episodes, param_env_bounds, reward_bounds=None, seed=None, teacher_params={}):
        ...
        elif teacher == 'ProCuRL-Target':
            self.task_generator = ProCuRLTargetAdapter(
    ...
    def record_train_episode(self, reward, index=0):
        ...
        if self.teacher != 'Oracle':
            if self.reward_bounds and self.teacher not in RAW_REWARD_TEACHERS:
                reward = np.interp(reward, self.reward_bounds, (0, 1))
            if self.reward_bounds:
                norm_reward = np.interp(raw_reward, self.reward_bounds, (0, 1))
            else:
                norm_reward = reward
            self.env_train_norm_rewards.append(norm_reward)
    ...
    def record_train_step(self, state, action, reward, next_state, done):
        if hasattr(self.task_generator, 'update_step'):
            self.task_generator.update_step(state, action, reward, next_state, done)
