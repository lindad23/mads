...
class MultigridNetwork(DeviceAwareModule):
    ...
    def __init__(self, 
        ...
        self.policy_feature_dim = 0
        obs_spaces = getattr(observation_space, 'spaces', observation_space)
        if 'policy_feature' in obs_spaces:
            self.policy_feature_dim = obs_spaces['policy_feature'].shape[0]
            self.preprocessed_input_size += self.policy_feature_dim
    ...
    def _forward_base(self, inputs, rnn_hxs, masks):
        ...
        in_policy_feature = inputs.get(
            'policy_feature',
            torch.tensor([], device=self.device))
        if self.policy_feature_dim and in_policy_feature.numel() > 0:
            in_policy_feature = in_policy_feature.float().view(
                in_policy_feature.shape[0], -1)
        ...
        in_embedded = torch.cat(
            (in_image, in_x, in_y, in_scalar, in_z, in_policy_feature),
            dim=-1)
