...
class BipedalWalkerAdversaryPolicy(DeviceAwareModule):
    def __init__(self, observation_space, action_space, editor=False, random=False, base_kwargs=None):
        ...
        # 兼容 Gym Space 对象和普通字典
        if hasattr(observation_space, 'spaces'):
            current_spaces = observation_space.spaces
        else:
            current_spaces = observation_space

        # 检查是否包含 'policy_feature' (这是 MADS 环境独有的)
        self.use_only_alpha = False
        
        if 'policy_feature' in current_spaces:
            # [Case A: MADS 模式]
            print("Detected 'policy_feature', switching to Alpha-Only input mode.")
            self.use_only_alpha = True
            obs_dim = current_spaces['policy_feature'].shape[0]
            # 为了防止报错，记录其他维度（虽然不用）
            self.design_dim = 0
            self.random_z_dim = 0
            
        else:
            # [Case B: PAIRED / Minimax 模式]
            # 保持原有逻辑不变
            self.design_dim = current_spaces['image'].shape[0]
            self.random_z_dim = current_spaces['random_z'].shape[0]
            # 输入维度 = 地图参数 + 随机噪声 + 时间步
    ...
    def preprocess(self, inputs):
        if self.use_only_alpha:
            # MADS: 直接返回 Alpha 特征，确保设备一致性
            policy_feature = inputs['policy_feature']
            # 处理 numpy 数组
            if isinstance(policy_feature, np.ndarray):
                policy_feature = torch.from_numpy(policy_feature).float()
            # 处理已存在的 tensor，确保设备一致
            if isinstance(policy_feature, torch.Tensor) and self.device is not None:
                policy_feature = policy_feature.to(self.device)
    ...
    def act(self, inputs, rnn_hxs, masks, deterministic=False):
        ...
        if self.random:
            if self.editor:
                ...
                # 对于连续动作，使用随机正态分布以保持适当的探索熵
                if self.use_only_alpha:
                    # MADS模式：使用截断正态分布限制在[-1,1]
                    action = torch.clamp(
                        torch.randn(inputs.shape[0], self.action_dim, device=self.device),
                        -1.0, 1.0
                    )
                    # 使用混合分布计算log_prob以保持非零熵
                    action_log_dist = torch.zeros(inputs.shape[0], self.action_dim, device=self.device)
                else:
