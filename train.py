...
if (
    ...
    and os.environ.get('MADS_SKIP_VIRTUAL_DISPLAY') != '1'
...
if __name__ == '__main__':
        ...
        if 'steps' in stats:
            for k, v in stats.items():
                if isinstance(v, (int, float, np.number)):
                    tb_writer.add_scalar(k, float(v), stats['steps'])
        if args.verbose:
    ...
    if args.cuda:
        if args.cuda_device < 0 or args.cuda_device >= torch.cuda.device_count():
            raise ValueError(
                f"Invalid --cuda_device={args.cuda_device}; "
                f"available logical CUDA devices: {torch.cuda.device_count()}"
            )
        torch.cuda.set_device(args.cuda_device)
    device = torch.device(f"cuda:{args.cuda_device}" if args.cuda else "cpu")
    if 'cuda' in device.type:
        torch.backends.cudnn.benchmark = True
        print(f'Using CUDA logical device {args.cuda_device}\n')
    ...
    target_eval_env_names = args.test_env_names.split(',') if args.test_env_names else []

    def add_target_eval_alias(stats):
        if not target_eval_env_names:
            return
        target_returns = [
            stats.get(f'test_returns:{env_name}')
            for env_name in target_eval_env_names
        ]
        target_solved_rates = [
            stats.get(f'solved_rate:{env_name}')
            for env_name in target_eval_env_names
        ]
        valid_returns = [v for v in target_returns if isinstance(v, (int, float, np.number))]
        valid_solved_rates = [v for v in target_solved_rates if isinstance(v, (int, float, np.number))]
        stats['target_agent_return'] = float(np.mean(valid_returns)) if valid_returns else None
        stats['target_solved_rate'] = float(np.mean(valid_solved_rates)) if valid_solved_rates else None
