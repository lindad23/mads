...
##
# 只杀掉属于 majunjie 的 Xvfb 进程
## pkill -u majunjie Xvfb
...
if sys.platform.startswith('linux') and os.environ.get('MADS_SKIP_VIRTUAL_DISPLAY', '0').lower() not in ('1', 'true', 'yes'):
...
from envs.runners.mads_runner import MADSRunner 
...
if __name__ == '__main__':
    ...
    if args.env_name.startswith('MultiGrid'):
        # MADS conditions the curriculum designer on the dynamic learner's
        # policy feature. MultiGrid does not expose this field by default, so
        # enable it only for the MADS entrypoint to keep baseline runs isolated.
        args.use_mads_policy_feature = True
        args.mads_policy_feature_dim = 32 * 7
    
    # === Configure logging ==
    if args.xpid is None:
        args.xpid = "lr-%s" % time.strftime("%Y%m%d-%H%M%S")
    _arg_log_dir = args.log_dir
    _script_dir = os.path.dirname(os.path.abspath(__file__))
    if isinstance(_arg_log_dir, str) and _arg_log_dir.startswith('~'):
        _suffix = os.path.basename(os.path.normpath(_arg_log_dir))
        log_dir = os.path.join(_script_dir, 'logs', _suffix)
    else:
        log_dir = os.path.expandvars(os.path.expanduser(_arg_log_dir))
    filewriter = FileWriter(
        xpid=args.xpid, xp_args=args.__dict__, rootdir=log_dir
    )
    tb_writer = SummaryWriter(os.path.join(log_dir, args.xpid, 'tb'))
        ...
        if 'steps' in stats:
            for k, v in stats.items():
                if isinstance(v, (int, float, np.number)):
                    tb_writer.add_scalar(k, v, stats['steps'])
        if args.verbose:
    ...
    if args.cuda and args.cuda_device >= torch.cuda.device_count():
        raise ValueError(
            f"Requested cuda:{args.cuda_device}, but only {torch.cuda.device_count()} CUDA device(s) are visible."
        )
    device = torch.device(f"cuda:{args.cuda_device}" if args.cuda else "cpu")
    ...
    for j in trange(initial_update_count, num_updates, desc="训练进度", unit="steps"):
