...
class CarRacingBezier(gym.Env, EzPickle):
    ...
    def __init__(self,
        ...
        self.track_x_scale = 1.0
        self.track_y_scale = 1.0
        ...
        self.adversary_observation_space = spaces.Dict({
            'time_step': spaces.Box(low=0, high=1, shape=(1,), dtype='uint8'),
            'random_z': spaces.Box(low=0, high=1, shape=(1,), dtype=np.float32),
        })
    ...
    def _create_track_bezier(self, control_points=None, show_borders=None):
        ...
        if self.preloaded_track is not None:
            points = np.array(self.preloaded_track.xy, dtype=np.float32)
            center = points.mean(axis=0, keepdims=True)
            scale = np.array([self.track_x_scale, self.track_y_scale], dtype=np.float32)
        ...
        for i in range(len(track)):
            ...
            t.road_friction = self.road_friction
