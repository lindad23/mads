...
class AdversarialRunner(object):
    ...
    def __init__(
        ...
        # Set up parameter-space curriculum teachers.
        if self.is_teacher_curriculum:
            self._init_teacher_curriculum()
    ...
    def _flat_space_dim(self, space):
        if isinstance(space, dict):
            return int(sum(self._flat_space_dim(s) for s in space.values()))
        if hasattr(space, 'shape') and space.shape is not None:
            return int(np.prod(space.shape))
        if hasattr(space, 'n'):
            return 1
        if hasattr(space, 'spaces'):
            return int(sum(self._flat_space_dim(s) for s in space.spaces.values()))
        raise ValueError(f'Unsupported space for curriculum baseline: {space}')

    def _physical_params_to_teacher_actions(self, values, ranges):
        actions = []
        for idx, value in enumerate(values, start=1):
            low, high = ranges[idx]
            denom = max(high - low, 1e-8)
            action = 2.0 * (float(value) - low) / denom
            actions.append(np.clip(action, 0.0, 2.0))
        return np.asarray(actions, dtype=np.float32)

    def _teacher_target_point(self, env_name):
        if env_name.startswith('Bipedal'):
            from envs.bipedalwalker.adversarial import (
                PARAM_RANGES_FULL,
                POET_PARAM_RANGES_FULL,
            ...
            if 'POET-Rose-' in env_name:
                rose_id = env_name.split('POET-Rose-', 1)[1].split('-v', 1)[0]
                if rose_id in POET_ROSE_LEVEL_PARAMS:
                    return self._physical_params_to_teacher_actions(
                        POET_ROSE_LEVEL_PARAMS[rose_id], POET_PARAM_RANGES_FULL)
                return None
            ...
            if 'Hard' in env_name:
                ...
                task = level_2_task
            elif 'Task0' in env_name:
                task = level_0_task
            elif 'Task1' in env_name:
                task = level_1_task
            else:
                task = level_1_task
            return self._physical_params_to_teacher_actions(task, PARAM_RANGES_FULL)

        if env_name.startswith('CarRacing') and 'MADS' in env_name:
            from envs.box2d.car_racing_f1 import F1_MADS_DEFAULT_PARAMS, F1_MADS_PARAM_RANGES
            return self._physical_params_to_teacher_actions(
                F1_MADS_DEFAULT_PARAMS, F1_MADS_PARAM_RANGES)

        return None

    def _teacher_target_samples(self, param_env_bounds):
        target_point = self._teacher_target_point(self.args.env_name)
        if target_point is None:
            return None

        expected_dim = 0
        for bounds in param_env_bounds.values():
            expected_dim += bounds[2] if len(bounds) == 3 else 1
        if len(target_point) != expected_dim:
            return None

        num_samples = self.args.curriculum_num_target_samples
        return np.repeat(target_point.reshape(1, -1), num_samples, axis=0).astype(np.float32)

    def _init_teacher_curriculum(self):
        args = self.args
        param_env_bounds = []
        if args.env_name.startswith('MultiGrid'):
            param_env_bounds = {'actions':[0,168,26]}
            reward_bounds = None
        elif args.env_name.startswith('CarRacing') and 'MADS' in args.env_name:
            param_env_bounds = {'actions': [0, 2, 4]}
            reward_bounds = None
            ...
            raise ValueError(f'Environment {args.env_name} not supported for teacher curriculum')
        ...
        if args.ued_algo in ['procurl_target', 'cp_drl']:
            teacher_params.update({
                ...
                'num_target_samples': args.curriculum_num_target_samples,
                'retrain_interval_episodes': args.curriculum_retrain_interval_episodes,
                'device': str(self.device),
            })
            target_samples = self._teacher_target_samples(param_env_bounds)
            if target_samples is not None:
                teacher_params['target_samples'] = target_samples
        if args.ued_algo == 'procurl_target':
            teacher_params['beta'] = args.procurl_beta
        elif args.ued_algo == 'cp_drl':
            teacher_params.update({
                'beta': args.cp_drl_beta,
                'transition_scale': args.cp_drl_transition_scale,
                'reward_scale': args.cp_drl_reward_scale,
                'state_scale': args.cp_drl_state_scale,
                'action_scale': args.cp_drl_action_scale,
                'aligned': args.cp_drl_aligned,
                'ensemble_size': args.cp_drl_ensemble_size,
                'model_batch_size': args.cp_drl_model_batch_size,
                'state_dim': self._flat_space_dim(self.venv.observation_space),
                'action_dim': self._flat_space_dim(self.venv.action_space),
            })

        self.curriculum_teacher = TeacherController(
        ...
        self.alp_gmm_teacher = self.curriculum_teacher

    def _to_numpy_batch(self, value):
        if isinstance(value, dict):
            parts = [self._to_numpy_batch(value[k]) for k in sorted(value.keys())]
            return np.concatenate(parts, axis=1)
        if torch.is_tensor(value):
            value = value.detach().cpu().numpy()
        arr = np.asarray(value)
        if arr.ndim == 0:
            arr = np.repeat(arr.reshape(1, 1), self.args.num_processes, axis=0)
        elif arr.shape[0] != self.args.num_processes:
            arr = arr.reshape(self.args.num_processes, -1)
        else:
            arr = arr.reshape(self.args.num_processes, -1)
        return arr.astype(np.float32)

    def _record_curriculum_steps(self, state, action, reward, next_state, done):
        if self.args.ued_algo != 'cp_drl':
            return
        states = self._to_numpy_batch(state)
        actions = self._to_numpy_batch(action)
        rewards = self._to_numpy_batch(reward).reshape(self.args.num_processes, -1)[:, 0]
        next_states = self._to_numpy_batch(next_state)
        dones = np.asarray(done).reshape(self.args.num_processes)
        for i in range(self.args.num_processes):
            self.curriculum_teacher.record_train_step(
    ...
    def agent_rollout(self, 
        ...
        if is_env:
            ...
            if level_replay: # Get replay levels
                ...
                self.total_seeds_collected += args.num_processes
                return
            elif self.is_teacher_curriculum:
                obs = self.curriculum_teacher.set_env_params(self.venv)
                self.total_seeds_collected += args.num_processes
                return
            else:
        ...
        for step in range(num_steps):
            ...
            if is_env:
                ...
                self._record_curriculum_steps(obs_id, _action, reward, obs, done)
            ...
            for i, info in enumerate(infos):
                if 'episode' in info.keys():
                    ...
                    if not is_env:
                        ...
                        # If using a parameter-space curriculum, sample next level.
                        if self.is_teacher_curriculum:
                            self.curriculum_teacher.record_train_episode(rollout_returns[i][-1], index=i)
                            self.curriculum_teacher.set_env_params(self.venv)
