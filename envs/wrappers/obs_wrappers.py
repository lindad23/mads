...
class VecPreprocessImageWrapper(VecEnvWrapper):
	...
	# MADS: [新增] 中间层转发接口 ===
	def set_policy_feature(self, feature):
		"""将策略特征(feature)转发给底层环境
		
		Args:
			feature: 策略特征向量 (numpy array 或 tensor)
			
		Raises:
			AttributeError: 如果底层环境不支持 set_policy_feature 方法
		"""
		if hasattr(self.venv, 'set_policy_feature'):
			self.venv.set_policy_feature(feature)
		else:
			raise AttributeError(
				f"Underlying venv {type(self.venv)} does not support set_policy_feature. "
				f"MADS mode requires BipedalWalkerMADS environment."
			)
