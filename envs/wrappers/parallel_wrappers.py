...
def worker(remote, parent_remote, env_fn_wrappers):
    ...
    try:
        while True:
            ...
            # MADS: 处理设置特征的指令 ===
            elif cmd == 'set_policy_feature':
                for env in envs: 
                    # 尝试找到底层的 env 并赋值
                    target = env.unwrapped if hasattr(env, 'unwrapped') else env
                    if hasattr(target, 'set_policy_feature'):
                        target.set_policy_feature(data)
                    else:
                        target.current_policy_feature = data
                # 发送确认信号，确保主进程知道特征已更新
                remote.send(True)
            # ============================== 
...
class ParallelAdversarialVecEnv(SubprocVecEnv):
    ...
    def get_level(self):
        ...
        result = []
        for level in levels:
            if isinstance(level, (list, tuple, np.ndarray)) and len(level) == 1:
                inner = level[0]
    ...
    # MADS: 发送 Alpha 特征给所有子进程 ===
    def set_policy_feature(self, feature):
        """将策略特征(feature)广播给所有子进程环境
        
        Args:
            feature: 策略特征向量 (numpy array)
        ...
        # 遍历所有子进程管道，发送指令和数据
        for remote in self.remotes:
            remote.send(('set_policy_feature', feature))
