...
from TeachMyAgent.environments.envs.Box2D_dynamics.water_dynamics import (
    WaterDynamics,
    WaterContactDetector,
)
from TeachMyAgent.environments.envs.Box2D_dynamics.climbing_dynamics import (
    ClimbingDynamics,
    ClimbingContactDetector,
)
from TeachMyAgent.environments.envs.PCGAgents.CPPN.TanHSoftplusMixCPPN import (
    TanHSoftplusMixCPPN,
)
    ...
    def BeginContact(self, contact):
        ...
        if any(
            ...
            for idx, body in enumerate(bodies):
                if (
                    body.userData.object_type == CustomUserDataObjectTypes.BODY_OBJECT
                    and body.userData.check_contact
                ):
                    body.userData.has_contact = True
                    other_body = bodies[(idx + 1) % 2]
                    # Authorize climbing bodies to touch climbing parts
                    if body.userData.is_contact_critical and not (
                        other_body.userData.object_type
                        == CustomUserDataObjectTypes.GRIP_TERRAIN
                        and self.env.agent_body.body_type == BodyTypesEnum.CLIMBER
                    ):
...
class LidarCallback(Box2D.b2.rayCastCallback):
    ...
    def ReportFixture(self, fixture, point, normal, fraction):
        ...
        self.is_water_detected = (
            True
            if fixture.body.userData.object_type == CustomUserDataObjectTypes.WATER
            else False
        )
        self.is_creeper_detected = (
            True
            if fixture.body.userData.object_type
...
class ParametricContinuousParkour(gym.Env, EzPickle):
    ...
    def __init__(
        ...
        **walker_args
        ...
        if body_type == BodyTypesEnum.SWIMMER or body_type == BodyTypesEnum.AMPHIBIAN:
            self.agent_body = BodiesEnum[agent_body_type].value(
                SCALE, density=WATER_DENSITY, **walker_args
            )
        elif body_type == BodyTypesEnum.WALKER:
            self.agent_body = BodiesEnum[agent_body_type].value(
                SCALE, **walker_args, reset_on_hull_critical_contact=False
            )
        else:
        ...
        self.TERRAIN_STARTPAD = (
            INITIAL_TERRAIN_STARTPAD
            if self.agent_body.AGENT_WIDTH / TERRAIN_STEP + 5
            <= INITIAL_TERRAIN_STARTPAD
            else self.agent_body.AGENT_WIDTH / TERRAIN_STEP + 5
        )  # in steps
        ...
        if CPPN_weights_path is None:
            ...
            CPPN_weights_path = os.path.join(
                current_path, "PCGAgents/CPPN/weights/same_ground_ceiling_cppn/"
            )

        self.terrain_CPPN = TanHSoftplusMixCPPN(
            x_dim=TERRAIN_LENGTH,
            input_dim=input_CPPN_dim,
            weights_path=CPPN_weights_path,
            output_dim=2,
        )  # ground + ceiling

        self.set_terrain_cppn_scale(
            terrain_cppn_scale, ceiling_offset, ceiling_clip_offset
        )
        ...
        )  # lidar info + if creeper for each lidar
    ...
    def set_terrain_cppn_scale(
        self, terrain_cppn_scale, ceiling_offset, ceiling_clip_offset
    ):
        """
    ...
    def set_environment(
        ...
        water_level,
        creepers_width=None,
        creepers_height=None,
        creepers_spacing=0.1,
        terrain_cppn_scale=10,
    ):
        """
        ...
        self.water_level = (
            water_level.item() if isinstance(water_level, np.float32) else water_level
        )
        self.water_level = max(0.01, self.water_level)
        self.creepers_width = (
            creepers_width if creepers_width is not None else creepers_width
        )
        self.creepers_height = (
            creepers_height if creepers_height is not None else creepers_height
        )
    ...
    def reset(self):
        ...
        self.lidar = [
            LidarCallback(
                self.agent_body.reference_head_object.fixtures[0].filterData.maskBits
            )
    ...
    def step(self, action):
        ...
        if (
            hasattr(self.agent_body, "nb_steps_can_survive_outside_water")
            and self.nb_steps_outside_water
            > self.agent_body.nb_steps_can_survive_outside_water
            or hasattr(self.agent_body, "nb_steps_can_survive_under_water")
            and self.nb_steps_under_water
            > self.agent_body.nb_steps_can_survive_under_water
        ):
            is_agent_dead = True
        ...
        if self.agent_body.body_type == BodyTypesEnum.CLIMBER:
            self.climbing_dynamics.before_step_climbing_dynamics(
                action, self.agent_body, self.world
            )
        ...
        if self.agent_body.body_type == BodyTypesEnum.CLIMBER:
            self.climbing_dynamics.after_step_climbing_dynamics(
                self.world.contactListener, self.world
            )
        ...
        for i in range(NB_LIDAR):
            ...
            self.lidar[i].p2 = (
                pos[0]
                + math.sin((self.lidar_angle * i / NB_LIDAR + self.lidar_y_offset))
                * LIDAR_RANGE,
                pos[1]
                - math.cos((self.lidar_angle * i / NB_LIDAR) + self.lidar_y_offset)
                * LIDAR_RANGE,
            )
        ...
        self.scroll = [
            pos[0] - RENDERING_VIEWER_W / SCALE / 5,
            pos[1] - RENDERING_VIEWER_H / SCALE / 5 - TERRAIN_HEIGHT + 1 / SCALE,
        ]  # 1 = grass
        ...
        if not (
            hasattr(self.agent_body, "remove_reward_on_head_angle")
            and self.agent_body.remove_reward_on_head_angle
        ):
            shaping -= 5.0 * abs(
                state[0]
        ...
        for a in action:
            reward -= (
                self.agent_body.TORQUE_PENALTY * 80 * np.clip(np.abs(a), 0, 1)
            )  # 80 => Original torque
        ...
        if (
            pos[0]
            > (TERRAIN_LENGTH + self.TERRAIN_STARTPAD - TERRAIN_END) * TERRAIN_STEP
        ):
            done = True
    ...
    def render(self, mode="human", draw_lidars=True):
        ...
        self.viewer.set_bounds(
            self.scroll[0],
            RENDERING_VIEWER_W / SCALE + self.scroll[0],  # x
            self.scroll[1],
            RENDERING_VIEWER_H / SCALE + self.scroll[1],
        )  # y
        ...
        self.viewer.draw_polygon(
            [
                ...
                (
                    self.scroll[0] + RENDERING_VIEWER_W / SCALE,
                    self.scroll[1] + RENDERING_VIEWER_H / SCALE,
                ),
        ...
        for poly, x1, x2 in self.cloud_poly:
            if x2 < self.scroll[0] / 2:
                continue
            if x1 > self.scroll[0] / 2 + RENDERING_VIEWER_W / SCALE:
                continue
            self.viewer.draw_polygon(
                [(p[0] + self.scroll[0] / 2, p[1]) for p in poly], color=(1, 1, 1)
            )
        ...
        for obj in self.drawlist:
            ...
            color2 = obj.color2
            if (
                obj.userData.object_type == CustomUserDataObjectTypes.BODY_SENSOR
                and obj.userData.has_joint
            ):  # Color sensors when attached
                color1 = (1.0, 1.0, 0.0)
                color2 = (1.0, 1.0, 0.0)
            ...
            for f in obj.fixtures:
                ...
                if type(f.shape) is circleShape:
                    ...
                    self.viewer.draw_circle(f.shape.radius, 30, color=color1).add_attr(
                        t
                    )
                    self.viewer.draw_circle(
                        f.shape.radius, 30, color=color2, filled=False, linewidth=2
                    ).add_attr(t)
                else:
        ...
        for poly, color in self.terrain_poly:
            if poly[1][0] < self.scroll[0]:
                continue
            if poly[0][0] > self.scroll[0] + RENDERING_VIEWER_W / SCALE:
                continue
    ...
    def _generate_terrain(self):
        ...
        while x < max_x:
            ...
            if i < self.TERRAIN_STARTPAD:
                ...
                # Clip ceiling
                if (
                    ceiling_y[i - self.TERRAIN_STARTPAD]
                    >= ground_y[i - self.TERRAIN_STARTPAD] + self.ceiling_clip_offset
                ):
        ...
        for i in range(len(self.terrain_x) - 1):
            ...
            if self.creepers_width is not None and self.creepers_height is not None:
                if space_from_precedent_creeper >= self.creepers_spacing:
                    ...
                    creeper_step_size = np.floor(creeper_width / TERRAIN_STEP).astype(
                        int
                    )
                    ...
                    creeper_y_init_pos = max(
                        ...
                        self.terrain_ceiling_y[
                            min(i + creeper_step_size, len(self.terrain_x) - 1)
                        ],
                    )
                    if (
                        self.movable_creepers
                    ):  # Break down creepers into multiple objects linked by joints
                        ...
                        for w in range(math.ceil(creeper_height)):
                            ...
                            rjd = revoluteJointDef(
                                ...
                                bodyB=t,
                                anchor=(
                                    self.terrain_x[i] + creeper_width / 2,
                                    creeper_y_init_pos - (w * 1),
                                ),
                                enableLimit=True,
                        ...
                        t = self.world.CreateStaticBody(
                            fixtures=self.fd_creeper,
                            userData=CustomUserData(
                                "creeper", CustomUserDataObjectTypes.SENSOR_GRIP_TERRAIN
                            ),
                        )
                        c = (0.437, 0.504, 0.375)
                        t.color1 = c
                    ...
                    space_from_precedent_creeper += (
                        self.terrain_x[i] - self.terrain_x[i - 1]
                    )
    ...
    def _generate_agent(self):
        ...
        init_y = (
            TERRAIN_HEIGHT + self.agent_body.AGENT_CENTER_HEIGHT
        )  # set y position according to the agent
