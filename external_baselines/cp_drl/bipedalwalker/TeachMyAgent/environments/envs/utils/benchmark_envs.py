...
if __name__ == "__main__":
    ...
    for _env in envs:
        ...
        for walker_type in _env["walker_type"]:
            print(
                "##### Benchmarking {0} with {1} body #####".format(
            ...
            for j in range(2):
                if j == 0:
                    print(
                        "## Fixed env ##"
                    )  # Because the first value equals to the min possible
                else:
                    print("## Random env ##")

                env_fn, param_env_bounds = (
                    EnvironmentArgsHandler.get_object_from_arguments(args)
                )
                Teacher = TeacherArgsHandler.get_object_from_arguments(
                    args, param_env_bounds
                )
                ...
                for w in range(REPEAT_STEPS):
                    ...
                    for i in range(STEPS):
                        ...
                        if d or i % STEPS_PER_EPISODE == 0:
                            ...
                            env.reset()
                    print(
                        "Playing {0} steps for the {1} time  took for {2} s".format(
                            STEPS, w + 1, time.time() - t
                        )
                    )
                    print(
                        "During this, the environment was reset {0} times".format(
                            nb_of_resets
                        )
                    )
