...
class ParametricContinuousStumpTracks(gym.Env, EzPickle):
    ...
    def __init__(self, walker_type, **walker_args):
        ...
        if body_type == BodyTypesEnum.SWIMMER or body_type == BodyTypesEnum.AMPHIBIAN:
            self.walker_body = BodiesEnum[walker_type].value(
                SCALE, density=1.0, **walker_args
            )
        elif body_type == BodyTypesEnum.WALKER:
            self.walker_body = BodiesEnum[walker_type].value(
                SCALE, **walker_args, reset_on_hull_critical_contact=True
            )
        else:
        ...
        self.TERRAIN_STARTPAD = (
            ...
            if self.walker_body.AGENT_WIDTH / TERRAIN_STEP + 5
            <= INITIAL_TERRAIN_STARTPAD
            else self.walker_body.AGENT_WIDTH / TERRAIN_STEP + 5
        )  # in steps
        ...
        high = np.array(
            [np.inf] * (agent_state_size + 4 + NB_LIDAR)  # head infos
        )  # lidars infos
    ...
    def set_environment(
        ...
        self.obstacle_spacing = (
            max(0.01, obstacle_spacing) if obstacle_spacing is not None else 8.0
        )
    ...
    def reset(self):
        ...
        self.lidar = [
            LidarCallback(
                self.walker_body.reference_head_object.fixtures[0].filterData.maskBits
            )
    ...
    def step(self, action):
        ...
        if not (
            hasattr(self.walker_body, "remove_reward_on_head_angle")
            and self.walker_body.remove_reward_on_head_angle
        ):
            shaping -= 5.0 * abs(
                state[0]
        ...
        for a in action:
            reward -= (
                self.walker_body.TORQUE_PENALTY * 80 * np.clip(np.abs(a), 0, 1)
            )  # 80 => Original torque
    ...
    def render(self, mode="human", draw_lidars=True):
        ...
        self.viewer.set_bounds(
            self.scroll,
            RENDERING_VIEWER_W / SCALE + self.scroll,
            0,
            RENDERING_VIEWER_H / SCALE,
        )
        ...
        for poly, x1, x2 in self.cloud_poly:
            if x2 < self.scroll / 2:
                continue
            if x1 > self.scroll / 2 + RENDERING_VIEWER_W / SCALE:
                continue
            self.viewer.draw_polygon(
        ...
        for poly, color in self.terrain_poly:
            if poly[1][0] < self.scroll:
                continue
            if poly[0][0] > self.scroll + RENDERING_VIEWER_W / SCALE:
                continue
    ...
    def _generate_terrain(self):
        ...
        while x < max_x:
            ...
            if state == GRASS and not oneshot:
                ...
                if x > max_startpad_x:
                    velocity += (
                        self.np_random.uniform(-self.roughness, self.roughness) / SCALE
                    )
                y += velocity
                ...
                stump_height = max(
                    0.05,
                    self.np_random.normal(self.stump_height[0], self.stump_height[1]),
                )
                stump_width = TERRAIN_STEP
                if self.stump_width is not None:
                    stump_width *= max(
                        0.05, np.random.normal(self.stump_width[0], self.stump_width[1])
                    )
                poly = [
                    (x, y),
                    (x + stump_width, y),
                ...
                if self.stump_rot is not None:
                    ...
                    rotation = np.clip(
                        self.np_random.normal(self.stump_rot[0], self.stump_rot[1]),
                        0,
                        2 * np.pi,
                    )
                ...
                t = self.world.CreateStaticBody(fixtures=self.fd_default_hexagon)
                t.color1, t.color2 = (
                    1.0,
                    np.clip(delta_pos[0][1] / 3, 0, 1),
                    np.clip(delta_pos[-1][1] / 3, 0, 1),
                ), (0.6, 0.6, 0.6)
                self.terrain.append(t)
                ...
                for height, width in zip(self.stump_seq[0::2], self.stump_seq[1::2]):
                    ...
                    t = self.world.CreateStaticBody(
                        ...
                        userData=CustomUserData(
                            "grass", CustomUserDataObjectTypes.TERRAIN
                        ),
                    )
                    t.color1, t.color2 = (1, 1, 1), (0.6, 0.6, 0.6)
                    self.terrain.append(t)
    ...
    def _generate_walker(self):
        ...
        if (
            hasattr(self.walker_body, "old_morphology")
            and self.walker_body.old_morphology
        ):
