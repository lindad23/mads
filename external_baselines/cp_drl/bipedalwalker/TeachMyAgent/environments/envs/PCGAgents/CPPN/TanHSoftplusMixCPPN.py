...
class TanHSoftplusMixCPPN(BaseCPPN):
    ...
    def generator(self):
        ...
        self.h1_weights = tf.Variable(
            tf.truncated_normal([self.input_dim + 1, 64], mean=0.0, stddev=2.0)
        )
        h1 = tf.nn.tanh(tf.matmul(self.input, self.h1_weights))

        self.h2_weights = tf.Variable(
            tf.truncated_normal([64, 64], mean=0.0, stddev=2.0)
        )
        ...
        self.h4_weights = tf.Variable(
            tf.truncated_normal([64, 64], mean=0.0, stddev=2.0)
        )
        h4 = tf.nn.softplus(tf.matmul(h3, self.h4_weights))

        self.output_weights = tf.Variable(
            tf.truncated_normal([64, self.output_dim], mean=0.0, stddev=2.0)
        )
