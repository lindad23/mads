...
class WaterDynamics(object):
    ...
    def intersection(self, cp1, cp2, s, e):
        ...
        return Box2D.b2Vec2((n1 * dp.x - n2 * dc.x) * n3, (n1 * dp.y - n2 * dc.y) * n3)
        ...
        for pair in fixture_pairs:
            ...
            if has_intersection:
                ...
                for i in range(len(intersection_points)):
                    ...
                    vel_dir = pair[1].body.GetLinearVelocityFromWorldPoint(
                        mid_point
                    ...
                    if (
                        drag_dot >= 0
                    ):  # normal points backwards - this is not a leading edge
                        # apply drag
                        drag_mag = (
                            drag_dot * self.drag_mod * edge_length * density * vel * vel
                        )
                        drag_mag = min(drag_mag, self.max_drag)
                        drag_force = drag_mag * -vel_dir
                        pair[1].body.ApplyForce(
                            force=drag_force, point=mid_point, wake=True
                        )
                        ...
                        pair[1].body.ApplyForce(
                            force=lift_force, point=mid_point, wake=True
                        )
                    ##### PUSH
                    ...
                    joints_to_check = [
                        joint_edge.joint
                        for joint_edge in body_to_check.joints
                    ...
                    for joint in joints_to_check:
                        if joint.lowerLimit < joint.angle < joint.upperLimit:
                            ...
                            lever_vector = (
                                world_center - anchor
                            )  # vector from pivot to point of application of the force
...
class WaterContactDetector(contactListener):
    ...
    def BeginContact(self, contact):
        if (
            contact.fixtureA.body.userData.object_type
            == CustomUserDataObjectTypes.WATER
            and contact.fixtureB.body.userData.object_type
            == CustomUserDataObjectTypes.BODY_OBJECT
        ):
            self.fixture_pairs.append((contact.fixtureA, contact.fixtureB))
        elif (
            contact.fixtureB.body.userData.object_type
            == CustomUserDataObjectTypes.WATER
            and contact.fixtureA.body.userData.object_type
            == CustomUserDataObjectTypes.BODY_OBJECT
        ):
            self.fixture_pairs.append((contact.fixtureB, contact.fixtureA))
    ...
    def EndContact(self, contact):
        if (
            ...
            self.fixture_pairs.remove((contact.fixtureA, contact.fixtureB))
        elif (
            contact.fixtureB.body.userData.object_type
            == CustomUserDataObjectTypes.WATER
            and contact.fixtureA.body.userData.object_type
            ...
            self.fixture_pairs.remove((contact.fixtureB, contact.fixtureA))
