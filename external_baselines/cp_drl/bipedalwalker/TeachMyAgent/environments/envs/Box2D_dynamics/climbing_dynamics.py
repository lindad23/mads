...
class ClimbingDynamics(object):
    def before_step_climbing_dynamics(self, actions, body, world):
        ...
        for i in range(len(body.sensors)):
            ...
            if action_to_check > 0:  # Check whether the sensor should grasp or release
                ...
                if (
                    sensor_to_check.userData.has_joint
                ):  # If release and it had a joint => destroy it
                    sensor_to_check.userData.has_joint = False
                    joint_to_destroy = next(
                        (
                            _joint.joint
                            for _joint in sensor_to_check.joints
                            if isinstance(_joint.joint, Box2D.b2RevoluteJoint)
                        ),
                        None,
                    )
    ...
    def after_step_climbing_dynamics(self, contact_detector, world):
        ...
        for sensor in contact_detector.contact_dictionaries:
            if (
                len(contact_detector.contact_dictionaries[sensor]) > 0
                and sensor.userData.ready_to_attach
                and not sensor.userData.has_joint
            ):
                ...
                if (
                    sensor.worldCenter[0] + radius > min(x_values)
                    and sensor.worldCenter[0] - radius < max(x_values)
                    and sensor.worldCenter[1] + radius > min(y_values)
                    and sensor.worldCenter[1] - radius < max(y_values)
                ):
        ...
        for idx, body in enumerate(bodies):
            if (
                ...
                if (
                    ...
                    or other_body.userData.object_type
