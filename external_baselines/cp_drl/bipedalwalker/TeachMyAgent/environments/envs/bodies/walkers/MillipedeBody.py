...
MAIN_BODY_POLYGONS = [[(-10, +10), (+10, +10), (+10, -10), (-10, -10)]]
...
class MillipedeBody(WalkerAbstractBody):
    ...
    def __init__(
        ...
        self.AGENT_HEIGHT = 20 / self.SCALE + self.LEG_H * 2 - self.LEG_DOWN
    ...
    def draw(self, world, init_x, init_y, force_to_center):
        MAIN_BODY_FIXTURES = [
            fixtureDef(
                shape=polygonShape(
                    vertices=[(x / self.SCALE, y / self.SCALE) for x, y in polygon]
                ),
                density=5.0,
                friction=0.1,
                categoryBits=0x20,
        ...
        init_x = (
            init_x - MAIN_BODY_BOTTOM_WIDTH / self.SCALE * self.nb_of_bodies / 2
        )  # initial init_x is the middle of the whole body
        ...
        for j in range(self.nb_of_bodies):
            ...
            main_body = world.CreateDynamicBody(
                position=(main_body_x, init_y), fixtures=MAIN_BODY_FIXTURES
            )
            ...
            main_body.userData = CustomBodyUserData(
                True, is_contact_critical=False, name="body"
            )
            ...
            if previous_main_body is not None:
                rjd = revoluteJointDef(
                    ...
                    bodyB=main_body,
                    anchor=(
                        main_body_x - MAIN_BODY_BOTTOM_WIDTH / self.SCALE / 2,
                        init_y,
                    ),
                    enableMotor=False,
                    enableLimit=True,
        ...
        self.reference_head_object = (
            previous_main_body  # set as head the rightmost body
        )
