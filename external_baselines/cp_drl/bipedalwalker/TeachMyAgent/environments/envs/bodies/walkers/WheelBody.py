...
class WheelBody(WalkerAbstractBody):
    ...
    def __init__(
        ...
        self.AGENT_HEIGHT = 20 * self.body_scale / self.SCALE + self.LEG_H * 4
        self.AGENT_CENTER_HEIGHT = (
            20 * self.body_scale / self.SCALE / 2 + self.LEG_H * 2
        )
    ...
    def draw(self, world, init_x, init_y, force_to_center):
        ...
        for j in [[0, -1, 0], [0, 1, 0], [-1, 0, np.pi / 2], [1, 0, np.pi / 2]]:
            ...
            leg = world.CreateDynamicBody(
                position=(leg_x, leg_y), angle=(j[2]), fixtures=LEG_FD
            )
            ...
            lower = world.CreateDynamicBody(
                position=(leg_x + j[0] * (self.LEG_H), leg_y + j[1] * (self.LEG_H)),
                angle=(j[2]),
                fixtures=LOWER_FD,
            )
