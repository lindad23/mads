...
class AbstractBody(object):
            ...
            if motor_info.check_contact:
                state.extend(
                    [
                        ...
                        ),  # If motor has check_contact=True, check the contact of the associated contact_body
                    ]
                )
            else:
                state.extend(
    ...
    def activate_motors(self, action):
        ...
        for i in range(len(self.motors)):
            self.motors[i].motorSpeed = float(
                self.motors[i].userData.speed_control * np.sign(action[i])
            )
            self.motors[i].maxMotorTorque = float(
                self.MOTORS_TORQUE * np.clip(np.abs(action[i]), 0, 1)
            )
