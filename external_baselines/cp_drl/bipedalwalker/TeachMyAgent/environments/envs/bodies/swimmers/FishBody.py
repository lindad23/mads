...
from TeachMyAgent.environments.envs.bodies.swimmers.SwimmerAbstractBody import (
    SwimmerAbstractBody,
)
...
# Head
HULL_POLYGONS = [(-20, +12), (+6, +12), (+15, +4), (+15, -4), (+6, -12), (-20, -12)]

BODY_P1 = [(-8, +9), (+8, +12), (+8, -12), (-8, -9)]

BODY_P2 = [(-8, +4), (+8, +9), (+8, -9), (-8, -4)]

# Tail
BODY_P3 = [(-4, +2), (+4, +4), (+4, -4), (-4, -2)]

FIN = [(-1, -10), (-1, +10), (+1, +10), (+1, -10)]
...
class FishBody(SwimmerAbstractBody):
    ...
    def __init__(self, scale, density, motors_torque=80, nb_steps_outside_water=600):
        ...
        super(FishBody, self).__init__(
            scale, motors_torque, density, nb_steps_outside_water
        )
    ...
    def draw(self, world, init_x, init_y, force_to_center):
        ...
        HULL_FD = fixtureDef(
            shape=polygonShape(
                vertices=[(x / self.SCALE, y / self.SCALE) for x, y in HULL_POLYGONS]
            ),
        ...
        hull = world.CreateDynamicBody(position=(init_x, init_y), fixtures=HULL_FD)
        ...
        BODY_P1_FD = fixtureDef(
            shape=polygonShape(
                vertices=[(x / self.SCALE, y / self.SCALE) for x, y in BODY_P1]
            ),
            density=self.DENSITY,
            restitution=0.0,
            categoryBits=0x20,
        ...
        body_p1 = world.CreateDynamicBody(
            position=(body_p1_x, init_y), fixtures=BODY_P1_FD
        )
        ...
        joint_motor.userData = CustomMotorUserData(
            SPEED_KNEE, True, contact_body=body_p1
        )
        ...
        BODY_P2_FD = fixtureDef(
            shape=polygonShape(
                vertices=[(x / self.SCALE, y / self.SCALE) for x, y in BODY_P2]
            ),
            density=self.DENSITY,
            restitution=0.0,
            categoryBits=0x20,
        ...
        body_p2 = world.CreateDynamicBody(
            position=(body_p2_x, init_y), fixtures=BODY_P2_FD
        )
        ...
        joint_motor = world.CreateJoint(rjd)
        joint_motor.userData = CustomMotorUserData(
            SPEED_KNEE, True, contact_body=body_p2
        )
        self.motors.append(joint_motor)
        ...
        BODY_P3_FD = fixtureDef(
            shape=polygonShape(
                vertices=[(x / self.SCALE, y / self.SCALE) for x, y in BODY_P3]
            ),
            density=self.DENSITY,
            restitution=0.0,
            categoryBits=0x20,
        ...
        body_p3 = world.CreateDynamicBody(
            position=(body_p3_x, init_y), fixtures=BODY_P3_FD
        )
        ...
        joint_motor = world.CreateJoint(rjd)
        joint_motor.userData = CustomMotorUserData(
            SPEED_KNEE, True, contact_body=body_p3
        )
        self.motors.append(joint_motor)
        ...
        FIN_FD = fixtureDef(
            shape=polygonShape(
                vertices=[(x / self.SCALE, y / self.SCALE) for x, y in FIN]
            ),
            density=self.DENSITY,
            restitution=0.0,
            categoryBits=0x20,
        ...
        for fin_pos in fin_positions:
            ...
            joint_motor = world.CreateJoint(rjd)
            joint_motor.userData = CustomMotorUserData(
                SPEED_KNEE, True, contact_body=fin
            )
            self.motors.append(joint_motor)
