...
from TeachMyAgent.environments.envs.bodies.climbers.ClimberAbstractBody import (
    ClimberAbstractBody,
)
...
class ClimbingProfileChimpanzee(ClimberAbstractBody):
    ...
    def __init__(self, scale, motors_torque=100, nb_steps_under_water=600):
        ...
        """
        super(ClimbingProfileChimpanzee, self).__init__(
            scale, motors_torque, nb_steps_under_water
        )
        ...
        self.AGENT_HEIGHT = (
            self.BODY_HEIGHT / self.SCALE
            + self.HEAD_HEIGHT / self.SCALE
            + 0.2
            + self.LEG_H * 2
            - self.LEG_DOWN
        )
    ...
    def draw(self, world, init_x, init_y, force_to_center):
        head = world.CreateDynamicBody(
            position=(
                init_x,
                init_y
                + self.BODY_HEIGHT / self.SCALE / 2
                + self.HEAD_HEIGHT / self.SCALE / 2
                + 0.2,
            ),
            fixtures=fixtureDef(
                shape=polygonShape(
        ...
        body = world.CreateDynamicBody(
            ...
            fixtures=fixtureDef(
                shape=polygonShape(
                    vertices=[
                        (x / self.SCALE, y / self.SCALE)
                        for x, y in [(-12, +25), (+12, +25), (+8, -20), (-8, -20)]
                    ]
                ),
                density=5.0,
                friction=0.1,
        ...
        for i in [+1, +1]:
            ...
            upper.color1 = (0.6 - i / 10.0, 0.3 - i / 10.0, 0.5 - i / 10.0)
            upper.color2 = (0.4 - i / 10.0, 0.2 - i / 10.0, 0.3 - i / 10.0)
            rjd = revoluteJointDef(
                bodyA=body,
                bodyB=upper,
            ...
            lower.userData = CustomBodyUserData(
                True, name="lower_leg", is_contact_critical=True
            )
        ...
        for j in [-1, -1]:
            ...
            upper.color1 = (0.6 - j / 10.0, 0.3 - j / 10.0, 0.5 - j / 10.0)
            upper.color2 = (0.4 - j / 10.0, 0.2 - j / 10.0, 0.3 - j / 10.0)
            rjd = revoluteJointDef(
                bodyA=body,
                bodyB=upper,
            ...
            lower.color1 = (0.6 - j / 10.0, 0.3 - j / 10.0, 0.5 - j / 10.0)
            lower.color2 = (0.4 - j / 10.0, 0.2 - j / 10.0, 0.3 - j / 10.0)
            rjd = revoluteJointDef(
                bodyA=upper,
                bodyB=lower,
            ...
            for u in range(nb_hand_parts):
                hand_part = world.CreateDynamicBody(
                    position=(
                        init_x,
                        initial_y + self.HAND_PART_H / 2 + self.HAND_PART_H * u,
                    ),
                    fixtures=HAND_PART_FD,
                )

                hand_part.color1 = (0.6 - j / 10.0, 0.3 - j / 10.0, 0.5 - j / 10.0)
                hand_part.color2 = (0.4 - j / 10.0, 0.2 - j / 10.0, 0.3 - j / 10.0)
                rjd = revoluteJointDef(
                    bodyA=prev_part,
                    bodyB=hand_part,
            ...
            hand_sensor_position = (
                init_x,
                initial_y + self.HAND_PART_H * nb_hand_parts,
            )
            ...
            hand_sensor_part.color1 = (
                ...
                0,
                0,
            )  # (0.6 - j / 10., 0.3 - j / 10., 0.5 - j / 10.)
            hand_sensor_part.color2 = (
                1,
                0,
                0,
            )  # (0.4 - j / 10., 0.2 - j / 10., 0.3 - j / 10.)

            self.sensors.append(hand_sensor_part)
            world.CreateJoint(
                weldJointDef(
                    bodyA=prev_part, bodyB=hand_sensor_part, anchor=hand_sensor_position
                )
            )
