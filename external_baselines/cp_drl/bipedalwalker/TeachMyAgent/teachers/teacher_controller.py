...
from TeachMyAgent.teachers.algos.currot import CurrotTeacher
from BipedalWalker.TeachMyAgent.teachers.algos.cp_drl import CPDRLTeacher
...
def param_dict_to_param_vec(param_env_bounds, param_dict):
    ...
    for name, bounds in param_env_bounds.items():
        if isinstance(param_dict[name], list) or isinstance(
            param_dict[name], np.ndarray
        ):
...
class TeacherController(object):
    ...
    def __init__(
        ...
        seed=None,
        test_set=None,
        keep_periodical_task_samples=None,
        shuffle_dimensions=False,
        scale_reward=False,
        **teacher_params
    ):
        """
        ...
        if shuffle_dimensions:
            ...
            if "initial_dist" in teacher_params:
                teacher_params["initial_dist"]["mean"] = (
                    self.dimensions_shuffler.inverse_interpolate_task(
                        teacher_params["initial_dist"]["mean"]
                    )
                )
        ...
        if teacher == "Random":
            ...
            self.task_generator = ADR(
                mins, maxs, seed=seed, scale_reward=scale_reward, **teacher_params
            )
        elif teacher == "Self-Paced":
            self.task_generator = SelfPacedTeacher(
                mins, maxs, seed=seed, **teacher_params
            )
            ...
            self.task_generator = CurrotTeacher(mins, maxs, seed=seed, **teacher_params)
        elif teacher == "CP-DRL":
            self.task_generator = CPDRLTeacher(mins, maxs, seed=seed, **teacher_params)
        else:
        ...
        if test_set is None:
            if (
                self.task_dim == 2
                and "stump_height" in param_env_bounds
                and "obstacle_spacing" in param_env_bounds
            ):  # StumpTracks
                ...
                test_param_vec = np.transpose(
                    [np.tile(d1, len(d2)), np.repeat(d2, len(d1))]
                )  # cartesian product
            else:
                print("Using random test set.")
                test_random_state = np.random.RandomState(
                    31
                )  # Seed a new random generator not impacting the global one to always get the same test set
                test_param_vec = test_random_state.uniform(
                    mins, maxs, size=(nb_test_episodes, self.task_dim)
                )
        else:
            test_param_vec = np.array(
                pickle.load(
                    open("TeachMyAgent/teachers/test_sets/" + test_set + ".pkl", "rb")
                )
            )
            self.nb_test_episodes = len(test_param_vec)
            print("fixed set of {} tasks loaded".format(len(test_param_vec)))
        test_param_dicts = [
            param_vec_to_param_dict(param_env_bounds, vec) for vec in test_param_vec
        ]
    ...
    def record_train_step(self, state, action, reward, next_state, done):
        ...
        # Monitor curriculum
        if (
            self.keep_periodical_task_samples is not None
    ...
    def record_train_episode(self, ep_reward, ep_len, is_success=False):
        ...
        self.task_generator.episodic_update(
            self._get_last_task(), ep_reward, is_success
        )
