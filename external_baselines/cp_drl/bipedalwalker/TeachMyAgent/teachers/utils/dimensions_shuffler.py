...
class DimensionsShuffler:
    def __init__(self, mins, maxs, cuttings=4, seed=21):
        ...
        for i in range(len(mins)):
            ...
            current_dim_dict = {
                "original": [
                    (current_min + j * region_size, current_min + (j + 1) * region_size)
                    for j in range(cuttings)
                ],
                "shuffled": [
                    (current_min + j * region_size, current_min + (j + 1) * region_size)
                    for j in range(cuttings)
    ...
    def interpolate_task(self, task):
        ...
        for i in range(self.nb_dims):
            try:
                region_index = next(
                    idx
                    for idx, v in enumerate(self.dims_dicts[i]["original"])
                    if v[0] <= task[i] <= v[1]
                )
            except StopIteration as err:
                raise Exception(
                    "Unable to find the index of the {} dimension of task {}.".format(
                        i, task
                    )
                ) from err
    ...
    def inverse_interpolate_task(self, task):
        ...
        for i in range(self.nb_dims):
            try:
                region_index = next(
                    idx
                    for idx, v in enumerate(self.dims_dicts[i]["shuffled"])
                    if v[0] <= task[i] <= v[1]
                )
            except StopIteration as err:
                raise Exception(
