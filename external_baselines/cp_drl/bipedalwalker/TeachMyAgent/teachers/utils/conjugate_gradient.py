...
def _fisher_vector_product_t(p, kl_fun, param_fun, cg_damping):
    ...
    grads_v = torch.autograd.grad(
        kl_v, param_fun(), create_graph=False, retain_graph=True
    )
...
def _conjugate_gradient(
    b, kl_fun, param_fun, cg_damping, n_epochs_cg, cg_residual_tol, use_cuda=False
):
    ...
    for i in range(n_epochs_cg):
        z = (
            _fisher_vector_product(p, kl_fun, param_fun, cg_damping, use_cuda=use_cuda)
            .detach()
            .cpu()
            .numpy()
        )
...
def cg_step(
    ...
    if np.linalg.norm(g) < 1e-10:
        ...
        if np.any(np.isnan(stepdir)) or np.any(np.isinf(stepdir)):
            raise RuntimeError(
                "Computation of conjugate gradient resulted in NaNs or Infs"
            )
...
def _line_search(
    ...
    direction = (
        _fisher_vector_product(
            stepdir, kl_fun, param_fun, cg_damping, use_cuda=use_cuda
        )
        .detach()
        .cpu()
        .numpy()
    )
    shs = 0.5 * stepdir.dot(direction)
