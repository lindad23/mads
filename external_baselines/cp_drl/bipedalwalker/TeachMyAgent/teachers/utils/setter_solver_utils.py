...
def non_zero_uniform(shape):
    ...
    return tf.where(tf.equal(rnd, 0.0), tf.ones_like(rnd) / 2.0, rnd)
...
def _softplus(x):
    ...
    return tf.where(x < -30.0, equiv_y, y)
    ...
    if n2 not in (n1, n1 - 1):
        raise ValueError(
            "z2 ({}) should be the same shape or 1 shorter than z1 ({}).".format(
                z2.get_shape().as_list(), z1.get_shape().as_list()
            )
        )
    if n2 < n1:
        # Add some padding.
...
class FlatRnvp(snt.AbstractModule):
    ...
    def __init__(
        ...
        self._optimizer = tf.train.AdamOptimizer(learning_rate=3e-4).minimize(
            self._loss, var_list=self._local_variables
        )
    ...
    def _build_infer(self, samples, condition):
        ...
        log_p = -tf.reduce_sum(self._final_non_linearity.log_differential(z), axis=1)
        log_p = tf.debugging.check_numerics(log_p, "initial_log_p")
        for block_layers in self._layers[::-1]:
            z, log_p = self._inverse_block_transform(z, log_p, condition, block_layers)
    ...
    def _build(self):
        ...
        self.sample_output, self.sample_log_p = self._build_sample(
            self._latent, self._sample_condition
        )
        self.infer_output, self.infer_log_p = self._build_infer(
            self._samples, self._infer_condition
        )
        self._local_variables = (
            tf.get_collection(  # used to avoid training judge's variables
                tf.GraphKeys.TRAINABLE_VARIABLES, self._original_name
            )
        )
    ...
    def _set_loss(self):
        feasibility_loss = tf.math.reduce_mean(
            tf.math.squared_difference(
                self._judge_output_op(self.sample_output),
    ...
    def sample(self, batch_size, condition=None):
        latent = self._random_state.normal(
            size=[batch_size, self._latent_size]
        )  # mean: 0, std: 1
        return self.tf_session.run(
            [self.sample_output, self.sample_log_p],
    ...
    def infer(self, samples, condition=None):
        ...
        return self.tf_session.run(
            [self.infer_output, self.infer_log_p],
    ...
    def train(self, samples, feasibilities, returns):
        ...
        noise = self._random_state.uniform(
            low=0, high=self._loss_noise_ub, size=succeeded_goals.shape
        )
        noisy_succeeded_goals = succeeded_goals + noise
        if self._logit_goals:
            noisy_succeeded_goals = np.clip(
                noisy_succeeded_goals, 1e-4, 1 - 1e-4
            )  # clip to match sigmoid

        latent = self.tf_session.run(
            self.infer_output,
            feed_dict={self._samples: samples, self._infer_condition: feasibilities},
        )
        return self.tf_session.run(
    ...
    def _block_transform(self, z, log_p, condition):
        ...
        self._layers.append([])
        z, log_p = self._left_to_right_transform(z, log_p, condition, self._transform)
        z, log_p = self._right_to_left_transform(z, log_p, condition, self._transform)
    ...
    def _inverse_block_transform(self, z, log_p, condition, block_layers):
        ...
        transform = functools.partial(self._inverse_transform, s_net=s_net, t_net=t_net)
...
class Judge(snt.AbstractModule):
    ...
    def __init__(
        self, hidden_sizes, tf_session, goal_size, custom_getter=None, name="judge"
    ):
    ...
    def _set_loss(self):
        ...
        return tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(
                labels=self._loss_returns, logits=self.output
            )
        )
    ...
    def train(self, returns, samples):
        return self.tf_session.run(
            [self._loss, self._optimizer],
            feed_dict={self._loss_returns: returns, self._goal: samples},
        )
