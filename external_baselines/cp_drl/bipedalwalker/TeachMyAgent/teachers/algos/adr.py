...
class ADR(AbstractTeacher):
    ...
    def episodic_update(self, task, reward, is_success):
        ...
        for i, (min_q, max_q, cur_min, cur_max) in enumerate(
            ...
            if (
                task[i] == cur_min
            ):  # if the proposed task has the i^th dimension set to min boundary
                min_q.append(reward)
                if len(min_q) == self.window_len:
                    if (
                        np.mean(min_q) >= self.max_reward_threshold
                    ):  # decrease min boundary (inflate sampling space)
                        self.cur_mins[i] = max(
                            self.cur_mins[i] - self.step_size, self.mins[i]
                        )
                    elif (
                        np.mean(min_q) <= self.min_reward_threshold
                    ):  # increase min boundary (deflate sampling space)
                        self.cur_mins[i] = min(
                            self.cur_mins[i] + self.step_size, self.cur_maxs[i]
                        )
                    self.min_queues[i] = deque(maxlen=self.window_len)  # reset queue
            if (
                task[i] == cur_max
            ):  # if the proposed task has the i^th dimension set to max boundary
                max_q.append(reward)
                if len(max_q) == self.window_len:  # queue is full, time to update
                    if (
                        np.mean(max_q) >= self.max_reward_threshold
                    ):  # increase max boundary
                        self.cur_maxs[i] = min(
                            self.cur_maxs[i] + self.step_size, self.maxs[i]
                        )
                    elif (
                        np.mean(max_q) <= self.min_reward_threshold
                    ):  # decrease max boundary
                        self.cur_maxs[i] = max(
                            self.cur_maxs[i] - self.step_size, self.cur_mins[i]
                        )
        ...
        if (prev_cur_mins != self.cur_mins).any() or (
            prev_cur_maxs != self.cur_maxs
        ).any():  # were boundaries changed ?
            self.task_space = Box(self.cur_mins, self.cur_maxs, dtype=np.float32)
            self.task_space.seed(self.seed)
    ...
    def sample_task(self):
        ...
        if (
            self.random_state.random() < self.bound_sampling_p
        ):  # set random dimension to min or max bound
