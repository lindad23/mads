...
class GoalGAN(AbstractTeacher):
    def __init__(
        ...
        self.success_buffer = StateCollection(
            1, success_distance_threshold * np.linalg.norm(self.maxs - self.mins)
        )
    ...
    def sample_task(self):
        if self.context_queue.empty():
            if self.random_state.random() > self.p_old or self.success_buffer.size == 0:
                ...
                context = np.clip(
                    context, self.mins, self.maxs, dtype=np.float32
                )  # added because the generators uses mins - 1e-6 and maxs - 1-6
    ...
    def non_exploratory_task_sampling(self):
        return {
            "task": np.float32(self.gan.sample_states(1)[0][0, :]),
            "infos": {
    ...
    def episodic_update(self, task, reward, is_success):
        ...
        if len(self.contexts) >= self.update_size:
            ...
            if np.any(labels):
                ...
                dis_log_loss, gen_log_loss = self.gan.train(
                    np.array(self.contexts), labels, 250
                )
