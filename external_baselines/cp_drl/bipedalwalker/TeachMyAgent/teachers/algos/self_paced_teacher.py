...
from TeachMyAgent.teachers.utils.gaussian_torch_distribution import (
...
class AbstractSelfPacedTeacher:
    ...
    def __init__(
        ...
        self.context_dist = GaussianTorchDistribution(
            init_mean, flat_init_chol, use_cuda=False
        )
        self.target_dist = GaussianTorchDistribution(
            target_mean, flat_target_chol, use_cuda=False
        )
...
class SelfPacedTeacher(AbstractTeacher, AbstractSelfPacedTeacher):
    def __init__(
        ...
        target_mean, target_variance = self.get_or_create_dist(
            target_dist, mins, maxs, subspace=False
        )  # Full task space if no intial dist
        ...
        AbstractSelfPacedTeacher.__init__(
            self,
            initial_mean,
            flat_init_chol,
            target_mean,
    ...
    def step_update(self, state, action, reward, next_state, done):
        ...
        if (
            self.step_counter >= self.update_offset
            and self.step_counter % self.update_frequency == 0
        ):
            if len(self.discounted_sum_rewards) > 0 and len(self.context_buffer) > 0:
                ...
                if values is None:
                    raise Exception(
                        "Please define a valid value estimator, this one returns None..."
                    )
                ...
                if self.use_avg_performance:
                    alpha_cur_t = self.alpha_function(
                        self.algorithm_iterations, avg_performance, cur_kl_t
                    )
                else:
                    alpha_cur_t = self.alpha_function(
                        self.algorithm_iterations,
                        torch.mean(c_val_t).detach(),
                        cur_kl_t,
                    )

                cg_step(
                    partial(
                        self._compute_context_loss,
                        contexts_t,
                        old_c_log_prob_t,
                        c_val_t,
                        alpha_cur_t,
                    ),
                    partial(self._compute_context_kl, old_context_dist),
                    self.max_kl,
                    self.context_dist.parameters,
                    self.context_dist.set_weights,
                    self.context_dist.get_weights,
                    **self.cg_parameters,
                    use_cuda=False
                )
                ...
                if (
                    self.std_lower_bound is not None
                    and self.target_context_kl() > self.kl_threshold
                ):
                    cov[0 : self.context_dim] = np.log(
                        np.maximum(
                            np.exp(cov[0 : self.context_dim]), self.std_lower_bound
                        )
                    )
                    self.context_dist.set_weights(
                        np.concatenate((self.context_dist.mean(), cov))
                    )
                ...
                print(
                    "Skipping iteration at step {} because buffers are empty.".format(
                        self.step_counter
                    )
                )
    ...
    def sample_task(self):
        ...
        return np.clip(
            sample, self.context_bounds[0], self.context_bounds[1], dtype=np.float32
        )
