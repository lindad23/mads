...
from TeachMyAgent.teachers.utils.setter_solver_utils import (
    Judge,
    FlatRnvp,
    ClippedSigmoid,
)
    ...
    def add(self, key, goal, feasibility, reward):
        if key in self.buffer:
            self.buffer[key].append(
                {"goal": goal, "feasibility": feasibility, "return": reward}
            )
        else:
            self.buffer[key] = [
                {"goal": goal, "feasibility": feasibility, "return": reward}
...
class SetterSolver(AbstractTeacher):
    ...
    def episodic_update(self, task, reward, is_success):
        ...
        if last_sampled_goal[0] == goal_key:
            self.goal_buffer.add(
                goal_key, last_sampled_goal[1], last_sampled_goal[2], is_success
            )
        else:
    ...
    def non_exploratory_task_sampling(self):
        ...
        sample, log_p = self.setter.sample(1, condition=feasibility)
        return {
            "task": np.float32(self.rescale_task(sample[0])),
            "infos": {
