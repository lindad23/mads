...
class AbstractTeacher(object):
    ...
    def create_dist_from_bounds(self, mins, maxs, subspace):
        ...
        if subspace:
            mean = np.array(
                [self.random_state.uniform(min, max) for min, max in zip(mins, maxs)]
            )
            variance = [
                (abs(max - min) * 0.1) ** 2 for min, max in zip(mins, maxs)
            ]  # std = 10 % of each dimension
        else:
            mean = np.array([np.mean([min, max]) for min, max in zip(mins, maxs)])
            variance = [
                (abs(max - min) / 4) ** 2 for min, max in zip(mins, maxs)
            ]  # std = 0.25 * range => ~95.5% of samples are between the bounds
        variance = [
            1e-6 if v == 0 else v for v in variance
        ]  # avoid errors with null variance
    ...
    def get_or_create_dist(self, dist_dict, mins, maxs, subspace=False):
        ...
        if dist_dict is not None:
            ...
            dist_mean, dist_variance = self.create_dist_from_bounds(
                mins, maxs, subspace
            )
    ...
    def rescale_task(self, task, original_space=(0, 1)):
        ...
        return np.array(
            [
                np.interp(task[i], original_space, (self.mins[i], self.maxs[i]))
    ...
    def inverse_rescale_task(self, task, original_space=(0, 1)):
        ...
        return np.array(
            [
                np.interp(task[i], (self.mins[i], self.maxs[i]), original_space)
                for i in range(len(self.mins))
            ]
        )
