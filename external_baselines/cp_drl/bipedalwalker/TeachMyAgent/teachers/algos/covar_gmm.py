...
class CovarGMM(AbstractTeacher):
    def __init__(
        ...
        self.nb_bootstrap = (
            nb_bootstrap if nb_bootstrap is not None else fit_rate
    ...
    def episodic_update(self, task, reward, is_success):
        ...
        if len(self.tasks) >= self.nb_bootstrap:  # If initial bootstrapping is done
            if (len(self.tasks) % self.fit_rate) == 0:  # Time to fit
                ...
                potential_gmms = [
                    GMM(n_components=k, covariance_type="full", random_state=self.seed)
    ...
    def sample_task(self):
        task_origin = None
        if (
            len(self.tasks) < self.nb_bootstrap
            or self.random_state.random() < self.random_task_ratio
            or self.gmm is None
        ):
            if (
                self.initial_dist and len(self.tasks) < self.nb_bootstrap
            ):  # bootstrap in initial dist
                ...
                task_origin = (
                    -2
                )  # -2 = task originates from initial bootstrap gaussian sampling
            else:
            ...
            idx = proportional_choice(
                self.times_rewards_covars, self.random_state, eps=0.0
            )
            task_origin = idx
