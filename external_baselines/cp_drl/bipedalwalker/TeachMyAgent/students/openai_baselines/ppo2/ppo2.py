...
from TeachMyAgent.students.openai_baselines.common import (
    explained_variance,
    set_global_seeds,
)
...
try:
    ...
    MPI = None
from TeachMyAgent.students.openai_baselines.ppo2.synchronous_runner import (
    SynchronousRunner,
)
...
def learn(
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    value_estimator_fn = lambda states: np.array(
        [model.value(state) for state in states]
    )
    ...
    def test_agent(n, state=None):
        ...
        for j in range(n):
            if Teacher:
                Teacher.set_test_env_params(eval_env.get_raw_env())
    ...
    for update in range(1, nupdates + 1):
        ...
        if (
            ...
            if Teacher:
                Teacher.dump(logger.get_dir() + "/env_params_save.pkl")
