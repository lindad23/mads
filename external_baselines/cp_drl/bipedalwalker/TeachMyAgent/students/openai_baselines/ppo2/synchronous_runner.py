...
class SynchronousRunner:
    ...
    def __init__(self, *, env, model, nsteps, gamma, lam, max_ep_len, Teacher):
        ...
        self.obs = np.zeros(
            (1,) + env.observation_space.shape, dtype=env.observation_space.dtype.name
        )
    ...
    def run(self, Teacher):
        ...
        for _ in range(self.nsteps):
            ...
            Teacher.record_train_step(
                self.prev_unscaled_obs, a[0], unscaled_reward, unscaled_o, d[0]
            )
            ...
            if d or self.ep_len == self.max_ep_len:
                ...
                epinfos.append({"r": self.raw_ep_ret, "l": self.ep_len})
                self.ep_len = 0
                self.ep_ret = 0
                self.raw_ep_ret = 0

            self.obs, self.prev_unscaled_obs, rewards, self.dones, infos = (
                o,
                unscaled_o,
                r,
                d,
