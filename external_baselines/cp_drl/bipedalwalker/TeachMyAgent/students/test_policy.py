...
from TeachMyAgent.students.spinup.utils.test_policy import (
    load_policy_and_env as spinup_load_policy,
)
from TeachMyAgent.students.openai_baselines.ppo2.ppo2 import (
    get_model as get_baselines_model,
)
    ...
    for f in listdir(path):
        if osp.isfile(osp.join(path, f)):
            try:
                checkpoint = int(f)
                last_checkpoint = (
                    f if checkpoint > int(last_checkpoint) else last_checkpoint
                )
            except Exception:
                continue
...
def load_training_test_set(save_path, order_by_best_rewards=None):
    ...
    if order_by_best_rewards is not None:
        print(
            "Getting test set tasks ordered by last return from {} to {} ...".format(
                "greatest" if order_by_best_rewards else "lowest",
                "lowest" if order_by_best_rewards else "greatest",
            )
        )
        sorted_indexes_of_test_episodes = sorted(
            range(test_set_size),
            key=lambda k: test_rewards_to_use[k],
            reverse=order_by_best_rewards,
        )  # Sort with best results first
    else:
    ...
    env_params_list = [
        param_vec_to_param_dict(teacher_param_env_bounds, test_params_to_use[i])
        for i in sorted_indexes_of_test_episodes
    ]
    associated_rewards_list = [
        test_rewards_to_use[i] for i in sorted_indexes_of_test_episodes
    ]
...
def load_fixed_test_set(save_path, test_set_name):
    ...
    test_param_vec = np.array(
        pickle.load(
            open("TeachMyAgent/teachers/test_sets/" + test_set_name + ".pkl", "rb")
        )
    )

    return [
        param_vec_to_param_dict(teacher_param_env_bounds, vec) for vec in test_param_vec
    ]
...
def run_policy(
    ...
    if record:
        ...
        video_recorder = VideoRecorder(
            env, recording_path + "_ep" + str(episode_id) + ".mp4", enabled=True
        )
...
def main(args):
    ...
    if student_type == "spinup":
        ...
        nbatch_train = (
            args.nb_env_steps * 1e6 // int(args.sample_size // args.batch_size)
        )

        model = get_baselines_model(
