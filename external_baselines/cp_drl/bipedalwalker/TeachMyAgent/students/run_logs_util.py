...
def get_spinup_run_logs(root, exp_idx, units, condition=None):
    ...
    if "AverageTestEpRet" in exp_data:
        exp_data.insert(
            len(exp_data.columns), "evaluation return", exp_data["AverageTestEpRet"]
        )
...
def get_baselines_run_logs(root, exp_idx, units, condition=None):
    ...
    if "eval_eprewmean" in exp_data:
        exp_data.insert(
            len(exp_data.columns), "evaluation return", exp_data["eval_eprewmean"]
        )
    ...
    for root, _, files in os.walk(logdir):
        ...
        if nb_epochs >= min_len:
            if "env_params_save.pkl" in files:
                try:
                    env_params_dict = pickle.load(
                        open(os.path.join(root, "env_params_save.pkl"), "rb")
                    )
                    ...
                    print(err)
                    print(
                        "Corrupted save, ignoring {}".format(root[-1])
                    )  # data_dict['config']['seed']))
