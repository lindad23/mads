...
class TwinCustomVecNormalizeEnv(CustomVecNormalizeEnv):
    ...
    def _rewsfilt(self, rews, news):
        ...
        if self.ret_rms:
            rews = np.clip(
                rews / np.sqrt(self.twin.ret_rms.var + self.epsilon),
                -self.cliprew,
                self.cliprew,
            )
        self.ret[news] = 0.0
    ...
    def _obfilt(self, obs):
        if self.ob_rms:
            obs = np.clip(
                (obs - self.twin.ob_rms.mean)
                / np.sqrt(self.twin.ob_rms.var + self.epsilon),
                -self.clipob,
                self.clipob,
            )
            return obs
...
def create_custom_vec_normalized_envs(env_f):
    ...
    vec_normalized_test_env = TwinCustomVecNormalizeEnv(
        vec_normalized_env, clipped_test_env, use_tf=False
    )
