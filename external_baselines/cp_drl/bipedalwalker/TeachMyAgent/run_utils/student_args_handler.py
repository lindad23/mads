...
class StudentArgsHandler(AbstractArgsHandler):
    ...
    def set_parser_arguments(parser):
        ...
        """
        parser.add_argument(
            "--student", type=str, default="sac_v0.1.1"
        )  # Currently : sac_v0.1.1, sac_v0.2, ppo
        parser.add_argument(
            "--backend", type=str, default="tf1"
        )  # Currently : tf1 / pytorch

        parser.add_argument("--network", type=str, default="custom_mlp")
        parser.add_argument(
            "--hidden_sizes", type=str, default="400/300"
        )  # layers (with nb of neurons) separated by '/'

        # For all students
        parser.add_argument("--gamma", type=float, default=0.99)
        parser.add_argument(
            "--nb_env_steps", type=float, default=20.0
        )  # Training time, expressed in Millions of env steps
        ...
        parser.add_argument(
            "--steps_per_ep", type=int, default=500000
        )  # nb env steps/epoch (stay above max_ep_len and nb_env_steps)
        ...
        # PPO specific
        parser.add_argument(
            "--sample_size", type=int, default=20000
        )  # size of trajectories sample between two updates
        parser.add_argument(
            "--epochs_per_update", type=int, default=5
        )  # how many times the model is trained on the full trajectories sample
        ...
        parser.add_argument(
            "--value_network", type=str, default="shared"
        )  # shared OR copy
    ...
    def get_object_from_arguments(args, env_f, teacher):
        ...
        if args.network == "lstm":
            ...
            if args.student == "ppo" and args.batch_size != args.sample_size:
                print(
                    "Setting batch size to the same value as sample size ({}) for ppo with lstm...".format(
                        args.sample_size
                    )
                )
        ...
        if "sac" in args.student:
            ...
            if args.student == "sac_v0.1.1":
                if args.backend == "tf1":
                    ...
                    raise (
                        "Old Spinup version of SAC only uses TF1. Please use the new version to use PyTorch."
                    )
                ...
                return lambda: sac(
                    ...
                    steps_per_epoch=args.steps_per_ep,
                    replay_size=args.buf_size,
                ...
                return lambda: sac(
                    ...
                    gamma=args.gamma,
                    seed=args.seed,
                    epochs=int((args.nb_env_steps * 1e6) // args.steps_per_ep),
                    start_steps=start_steps,
                    logger_kwargs=logger_kwargs,
                    alpha=args.alpha,
                    max_ep_len=args.max_ep_len,
                    steps_per_epoch=args.steps_per_ep,
                    replay_size=args.buf_size,
                    num_test_episodes=args.nb_test_episodes,
                    lr=args.lr,
                    update_every=args.train_freq,
                    batch_size=args.batch_size,
                    Teacher=teacher,
                    half_save=half_save,
                    pretrained_model=pretrained_model,
            ...
            if args.backend == "pytorch":
                raise (
                    "Currently only implemented OpenAI Baselines' version, which is TF1."
                )
            from TeachMyAgent.students.openai_baselines.ppo2.ppo2 import learn
            from TeachMyAgent.students.ppo_utils import (
                create_custom_vec_normalized_envs,
            )
            ...
            log_interval = int(
                args.steps_per_ep // args.sample_size
            )  # Log every "epoch"
            if args.no_save is True:
                save_interval = None
            else:
                if args.half_save is True:
                    ...
                    print(
                        "Warning: not using half_save with PPO will produce a lot of disk usage."
                    )
            ...
            return lambda: learn(
                ...
                env=env,
                eval_env=eval_env,
                total_timesteps=args.nb_env_steps * 1e6,
                nsteps=args.sample_size,
                nminibatches=int(args.sample_size // args.batch_size),
                lr=args.lr,
                noptepochs=args.epochs_per_update,
                ...
                max_grad_norm=args.max_grad_norm_coef,
                save_interval=save_interval,
                Teacher=teacher,
                max_ep_len=args.max_ep_len,
                nb_test_episodes=args.nb_test_episodes,
                hidden_sizes=ac_kwargs["hidden_sizes"],
                logger_dir=logger_kwargs["output_dir"],
                reset_frequency=args.reset_frequency,
                value_network=args.value_network,
            )
