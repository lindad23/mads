...
class TeacherArgsHandler(AbstractArgsHandler):
    ...
    def set_parser_arguments(parser):
        ...
        """
        parser.add_argument(
            "--teacher", type=str, default="ALP-GMM"
        )  # Random, ADR, ALP-GMM, Covar-GMM, RIAC, GoalGAN, Self-Paced, Setter-Solver
        parser.add_argument("--test_set", type=str, default=None)
        parser.add_argument(
            "--allow_expert_knowledge", type=str, default="original"
        )  # original (white paper's version), no, low, high
        parser.add_argument(
            "--keep_periodical_task_samples", type=int, default=None
        )  # in steps /!\
        ...
        parser.add_argument(
            "--alp_max_size", type=float, default=None
        )  # alp-computer window, expressed in Millions of env steps
    ...
    def get_object_from_arguments(
        ...
        if args.teacher == "ALP-GMM":
            ...
            if args.allow_expert_knowledge in ["original", "low", "high"]:
                ...
                raise Exception(
                    "Unable to run ADR without any expert knowledge (needs at least reward thresholds)."
                )
            params["queue_len"] = args.queue_len
            if initial_dist is not None and args.allow_expert_knowledge in [
                "original",
                "high",
            ]:
                params["initial_dist"] = initial_dist
            ...
            params["alpha_function"] = PercentageAlphaFunction(
                args.alpha_offset, args.zeta
            )
            ...
            if initial_dist is not None and args.allow_expert_knowledge in [
                ...
                params["initial_dist"] = initial_dist
            if target_dist is not None and args.allow_expert_knowledge in [
                "original",
                "maximal",
            ]:
            ...
            if args.allow_expert_knowledge == "no":
                raise Exception(
                    "Unable to run GoalGAN without any expert knowledge (needs at least a success definition)."
                )
            ...
            if args.use_pretrained_samples or args.allow_expert_knowledge in [
                "original",
                "high",
            ]:
            ...
            if args.allow_expert_knowledge == "no":
                raise Exception(
                    "Unable to run Setter-Solver without any expert knowledge (needs at least a success definition)."
                )
        ...
        elif args.teacher == "Currot":
            pass

        elif args.teacher == "CP-DRL":
        ...
        teacher = TeacherController(
            ...
            seed=args.seed,
            keep_periodical_task_samples=args.keep_periodical_task_samples,
            shuffle_dimensions=shuffle_dims,
        ...
        if args.test_set is not None:
            ...
            print(
                "Changing the number of test episodes to {0} with the test set {1}".format(
                    args.nb_test_episodes, args.test_set
                )
            )
        return teacher
