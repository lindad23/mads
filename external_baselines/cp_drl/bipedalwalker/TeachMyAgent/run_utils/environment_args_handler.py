...
class EnvironmentArgsHandler(AbstractArgsHandler):
    ...
    def get_body_wargs(args):
        ...
        if body_type == BodyTypesEnum.SWIMMER and args.outside_water is not None:
            ...
            body_type == BodyTypesEnum.WALKER or body_type == BodyTypesEnum.CLIMBER
        ) and args.under_water is not None:
    ...
    def set_parser_arguments(parser):
        ...
        """
        parser.add_argument(
            "--env", type=str, default="parametric-continuous-stump-tracks-v0"
        )

        # Choose student's embodiment
        parser.add_argument(
            "--embodiment", type=str, default="old_classic_bipedal"
        )  # choose morphology
    ...
    def get_object_from_arguments(cls, args):
        ...
        if args.env == "parametric-continuous-stump-tracks-v0":
            ...
            if args.max_obstacle_spacing is not None:
                ...
                initial_dist["variance"].append(
                    (args.max_obstacle_spacing * 0.1) ** 2
                )  # std = 10% of dimension
            if args.hexa_shape:
            ...
            env_f = lambda: gym.make(
                args.env, walker_type=args.embodiment, **cls.get_body_wargs(args)
            )
            ...
            param_env_bounds["water_level"] = [
                args.min_water_level,
                args.max_water_level,
            ]
            param_env_bounds["creepers_width"] = [
                args.min_creepers_w,
                args.max_creepers_w,
            ]
            param_env_bounds["creepers_height"] = [0, args.max_creepers_h]
            param_env_bounds["creepers_spacing"] = [0, args.max_creepers_spacing]
            param_env_bounds["terrain_cppn_scale"] = [
                args.terrain_cppn_scale,
                args.terrain_cppn_scale,
            ]
