...
def process_arg_string(
    expe_args,
):  # function to extract flagged (with a *) arguments as details for experience name
    details_string = ""
    processed_arg_string = expe_args.replace(
        "*", ""
    )  # keep a version of args cleaned from exp name related flags
...
slurm_confs = {
    "curta_inria_extra_long": "#SBATCH -p XXX\n" "#SBATCH -t 119:00:00\n",
...
shutil.copyfile(
    cur_path + "/" + filename,
    cur_path + "/campain_logs/scripts/" + date + "_" + filename,
)
...
if launch:
    print(
        "Creating and Launching slurm scripts given arguments from {}".format(filename)
    )
    # time.sleep(1.0)
expe_list = []
...
for expe_args in expe_list:
    ...
    run_args = expe_args.split(exp_name, 1)[
        1
    ]  # WARNING: assumes that exp_name comes after slurm_conf and nb_seeds in txt
    ...
    slurm_script_fullname = (
        cur_path + "/campain_logs/scripts/{}".format(exp_name) + ".sh"
    )
    ...
    with open(slurm_script_fullname, "w") as f:
        ...
        f.write(
            "#SBATCH -o campain_logs/jobouts/{}.sh.out\n"
        ...
        f.write(
            "cp $SLURM_JOB_NAME TeachMyAgent/data/{}/{}.sh\n".format(exp_name, exp_name)
        )
        f.write("# Add info about git status to exp dir\n")
        f.write('current_commit="$(git log -n 1)"\n')
        f.write(
            'echo "${{current_commit}}" > TeachMyAgent/data/{}/git_status.txt\n'.format(
                exp_name
            )
        )
        f.write("# Launch !\n")
        ...
        f.write('if [ "$i" = "{}" ]; then\n'.format(n_cpus))
        f.write(
            "taskset -c $cpus $EXP_INTERP run.py --exp_name {} {} --seed $COUNT".format(
                exp_name, gpu
            )
            + run_args
            + " &\n"
    ...
    if launch:
        for i in range(int(nb_seeds) // 8):
            ...
            subprocess.check_call(
                [
                    "sbatch",
                    "campain_logs/scripts/{}.sh".format(exp_name),
                    str((i * 8) + seed_offset_to_use),
                ]
            )
    if incremental:
