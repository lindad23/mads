...
class ContextualPointMass(Env):
    ...
    def step(self, action):
        ...
        info = {
            "success": np.linalg.norm(self._goal_state[0::2] - new_state[0::2]) < 0.25
        }

        return (
            new_state,
            np.exp(-0.6 * np.linalg.norm(self._goal_state[0::2] - new_state[0::2])),
            crash,
            info,
        )
    ...
    def render(self, mode="human"):
        ...
        self._viewer.line(
            np.array([3.9, 0.9]), np.array([4.1, 1.1]), color=(255, 0, 0), width=0.1
        )
        self._viewer.line(
            np.array([4.1, 0.9]), np.array([3.9, 1.1]), color=(255, 0, 0), width=0.1
        )
