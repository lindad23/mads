...
class VDS(AbstractTeacher):
    ...
    def initialize_teacher(self, env, learner, state_provider):
        ...
        self.replay_buffer = ReplayBuffer(
            self.q_train_config["replay_size"],
            obs_shape,
            action_dim,
        ...
        self.qs = EnsembleQFunction(
            **self.net_arch, input_dim=obs_shape[0] + action_dim, k=self.n_q
        )
        self.optimizer = torch.optim.Adam(
            self.qs.parameters(), lr=self.q_train_config["lr"]
        )
        ...
        if count >= self.next_update:
            ...
            for _ in range(self.q_train_config["n_epochs"]):
                batch_size = (
                    ...
                    // self.q_train_config["batches_per_epoch"]
                    + 1
                )
                for i in range(self.q_train_config["batches_per_epoch"]):
                    obs, acts, next_obs, dones, rewards = self.replay_buffer.sample(
                        (self.n_q, batch_size)
                    )
                    ...
                    with torch.no_grad():
                        next_q_values = self.qs(
                            torch.cat((next_obs, next_actions), axis=-1)
                        )
                    target_q_values = rewards + (1 - dones) * self.gamma * next_q_values
                    current_q_values = self.qs(torch.cat((obs, acts), axis=-1))
                    loss = torch.sum(
                        torch.nn.functional.mse_loss(current_q_values, target_q_values)
                    )
            ...
            disagreements = np.std(
                np.squeeze(
                    self.qs(torch.from_numpy(q_inputs).type(torch.float32))
                    .detach()
                    .numpy()
                ),
                axis=0,
    ...
    def sample(self):
        ...
        if self.qs is None or self.contexts is None:
            ...
            return self.contexts[
                np.random.choice(self.contexts.shape[0], p=self.likelihoods), ...
            ]
...
class EnsembleQFunction(torch.nn.Module):
    ...
    def __init__(self, input_dim, layers, act_func, k):
        ...
        for i in range(len(layers_ext) - 1):
            torch_layers.append(
                EnsembleLinear(layers_ext[i], layers_ext[i + 1], k, bias=True)
            )
...
class EnsembleLinear(torch.nn.Module):
    ...
    def __init__(
        self, in_features: int, out_features: int, k: int, bias: bool = True
    ) -> None:
...
class VDSWrapper(BaseWrapper):
    ...
    def __init__(
        self, env, vds, discount_factor, context_visible, context_post_processing=None
    ):
    ...
    def reset(self):
        ...
        if self.context_post_processing is None:
            ...
            self.processed_context = self.context_post_processing(
                self.cur_context
            ).copy()
    ...
    def step(self, action):
        ...
        self.teacher.replay_buffer.add(
            self.last_obs, step[0].copy(), action, step[1], step[2], []
        )
