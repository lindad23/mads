...
class PLR(AbstractTeacher):
    ...
    def sample_uniform(self):
        if self.is_discrete:
            to_sample = set(np.arange(self.context_lb, self.context_ub)) - set(
                self.contexts
            )
            if len(to_sample) == 0:
    ...
    def sample_prioritized(self):
        ...
        if cur_size == 1:
            ...
            staleness_probs = self.stalenesses[:cur_size] / np.sum(
                self.stalenesses[:cur_size]
            )
...
class PLRWrapper(BaseWrapper):
    ...
    def __init__(
        self, env, plr, discount_factor, context_visible, value_fn=None, lam=None
    ):
    ...
    def step(self, action):
        ...
        if step[2] and self.value_fn is not None:
            ...
            for cur_step in reversed(range(values.shape[0] - 1)):
                delta = (
                    self.reward_trace[cur_step]
                    + self.discount_factor * values[cur_step + 1]
                    - values[cur_step]
                )
                advantages[cur_step] = last_gae_lam = (
                    delta + self.discount_factor * self.lam * last_gae_lam
                )
            ...
            if self.value_fn.should_train(self.step_count):
                self.value_fn.train(
                    np.concatenate(self.train_state_buffer, axis=0),
    ...
    def done_callback(
        ...
        if self.value_fn is None:
            estimated_values = self.learner.estimate_value_internal(
                np.array(self.state_trace)
            )
        else:
...
class ValueFunction:
    ...
    def __init__(self, input_dim, layers, act_func, train_config):
        ...
        self.optimizer = torch.optim.Adam(
            self.model.parameters(), lr=self.train_config["lr"], eps=1e-5
        )
    ...
    def train(self, states, values):
        ...
        for _ in range(self.train_config["noptepochs"]):
            ...
            for epoch in range(self.train_config["minibatches"]):
                ...
                loss = torch.sum(
                    torch.nn.functional.mse_loss(predictions, values[batch_inds, :])
                )
...
class ValueModel(torch.nn.Module):
    ...
    def __init__(self, input_dim, layers, act_func):
        ...
        for i in range(len(layers_ext) - 1):
            torch_layers.append(
                torch.nn.Linear(layers_ext[i], layers_ext[i + 1], bias=True)
            )
