...
class Subsampler:
    ...
    def __init__(self, lb, ub, bins):
        eval_points = [
            np.linspace(lb[i], ub[i], bins[i] + 1)[:-1] for i in range(len(bins))
        ]
        eval_points = [s + 0.5 * (s[1] - s[0]) for s in eval_points]
        self.bin_sizes = np.array([s[1] - s[0] for s in eval_points])
        self.eval_points = np.stack(
            [
                m.reshape(
                    -1,
                )
                for m in np.meshgrid(*eval_points)
            ],
            axis=-1,
        )

    def __call__(self, discrete_sample):
        return self.eval_points[discrete_sample, :] + np.random.uniform(
            -0.5 * self.bin_sizes, 0.5 * self.bin_sizes
        )
...
class RewardEstimatorGP:
    ...
    def predict_individual(self, samples, with_gradient=False):
        ...
        if with_gradient:
            ...
            return perf.detach().numpy().astype(
                samples.dtype
    ...
    def __call__(self, samples, with_gradient=False):
        ...
        if with_gradient:
            ...
            return mean.detach().numpy().astype(
                samples.dtype
            ), grad.detach().numpy().astype(samples.dtype)
        else:
...
class NadarayaWatsonPy:
    ...
    def predict_individual(self, x, with_gradient=False):
        ...
        if with_gradient:
            ...
            return prediction, np.einsum(
                "...i,...ij->...j", weighted_returns, la_grads
