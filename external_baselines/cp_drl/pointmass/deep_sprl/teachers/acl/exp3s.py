...
class Exp3S:
    ...
    def normalize_reward(self, r):
        ...
        if np.isclose(self.min_reward, self.max_reward):
            ...
            return (
                2 * ((r_clip - self.min_reward) / (self.max_reward - self.min_reward))
                - 1
            )
    ...
    def update(self, i, r):
        ...
        self.count += 1
        alpha_t = 1.0 / self.count
        log_t1 = self.weights + np.log(1 - alpha_t)
        log_t2 = (
            self.logsumexp(self.weights)
            + np.log(alpha_t)
            - np.log(self.weights.shape[0])
        )
        log_t_max = np.maximum(log_t1, log_t2)
        self.weights = (
            np.log(np.exp(log_t1 - log_t_max) + np.exp(log_t2 - log_t_max)) + log_t_max
        )
