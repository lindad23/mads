...
class DistributionSampler(AbstractTeacher):
    ...
    def __init__(self, sample_func, lb, ub):
        ...
        n_samples = int(1e6)
        samples_not_ok = np.ones(n_samples, dtype=np.bool_)
        ...
        while np.any(samples_not_ok):
            ...
            samples_not_ok = np.logical_or(
                np.any(samples < lb, axis=-1), np.any(samples > ub, axis=-1)
            )
...
class DiscreteSampler(AbstractTeacher):
    ...
    def sample(self):
        return np.argmax(np.random.uniform(0.0, 1.0) <= np.cumsum(self.likelihoods))
