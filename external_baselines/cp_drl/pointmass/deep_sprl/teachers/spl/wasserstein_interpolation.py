...
class SamplingWassersteinInterpolation:
    ...
    def __init__(
        self, init_samples, target_sampler, perf_lb, epsilon, bounds, callback=None
    ):
    ...
    def sample_ball(self, targets, samples=None, half_ball=None, n=100):
        ...
        r = np.power(np.random.uniform(size=(n, 1)), 1.0 / self.dim)
        ...
        projected_noise = np.where(
            (noise_projections > 0)[..., None],
            noise[None, ...],
        ...
        return np.clip(
            samples[..., None, :] + scales * projected_noise,
            self.bounds[0],
            self.bounds[1],
        )
    ...
    def ensure_successful_initial(self, model, init_samples, success_samples):
        ...
        if n_replacements > 0:
            ...
            if n_valid >= n_replacements:
                ...
                valid_successes = np.ones(success_samples.shape[0], dtype=np.bool_)

            dists = np.sum(
                np.square(
                    success_samples[None, :, :]
                    - init_samples[replacement_mask, None, :]
                ),
                axis=-1,
            )
            success_assignment = linear_sum_assignment(dists, maximize=False)
            init_samples[replacement_mask, :] = success_samples[
                success_assignment[1], :
            ]
            performance_reached[replacement_mask] = valid_successes[
                success_assignment[1]
            ]
    ...
    def update_distribution(self, model, success_samples, debug=False):
        init_samples, performance_reached = self.ensure_successful_initial(
            model, self.current_samples.copy(), success_samples
        )
        ...
        particles = self.sample_ball(
            target_samples, samples=init_samples, half_ball=performance_reached
        )
        ...
        if debug:
            ...
            plt.scatter(
                target_samples_true[vis_idxs, 0],
                target_samples_true[vis_idxs, 1],
                marker="x",
                color="red",
            )
            plt.scatter(
                self.current_samples[vis_idxs, 0],
                self.current_samples[vis_idxs, 1],
                marker="o",
                color="C0",
            )
            plt.scatter(
                init_samples[vis_idxs, 0],
                init_samples[vis_idxs, 1],
                marker="o",
                color="C2",
            )
            plt.scatter(
                new_samples[vis_idxs, 0],
                new_samples[vis_idxs, 1],
                marker="o",
                color="C1",
            )
            plt.xlim([0, 9])
        ...
        if self.callback is not None:
            self.callback(
                self.current_samples, new_samples, success_samples, target_samples
            )
