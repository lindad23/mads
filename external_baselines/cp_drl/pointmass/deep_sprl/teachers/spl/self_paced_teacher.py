...
class AbstractSelfPacedTeacher:
    ...
    def __init__(
        ...
        self.context_dist = GaussianTorchDistribution(
            init_mean, flat_init_chol, use_cuda=False, dtype=torch.float64
        )
    ...
    def target_context_kl(self, numpy=True):
        ...
        kl = torch.mean(
            self.context_dist.log_pdf_t(samples)
        ...
        kl2 = torch.mean(
            torch.exp(cur_log_pdf - target_log_pdf) * (cur_log_pdf - target_log_pdf)
        )
...
class SelfPacedTeacher(AbstractTeacher, AbstractSelfPacedTeacher):
    ...
    def __init__(
        ...
        super(SelfPacedTeacher, self).__init__(
            initial_mean,
            flat_init_chol,
            target_log_likelihood,
            target_sampler,
    ...
    def old_kl_con(self, x, old_context_dist, obj=True, grad=False):
        dist = GaussianTorchDistribution.from_weights(
            self.context_dim, x, dtype=torch.float64
        )
        kl_div = torch.distributions.kl.kl_divergence(
            old_context_dist.distribution_t, dist.distribution_t
        )

        if grad:
            mu_grad, chol_flat_grad = torch.autograd.grad(kl_div, dist.parameters())
            dx = np.concatenate(
                [mu_grad.detach().numpy(), chol_flat_grad.detach().numpy()]
            )
            if obj:
    ...
    def expected_performance(
        ...
        perf = self._compute_expected_performance(
            dist, contexts_t, old_c_log_prob_t, c_val_t
        )
        if negate:
            perf = -1 * perf
    ...
    def update_distribution(self, contexts, values):
        old_context_dist = GaussianTorchDistribution.from_weights(
            self.context_dim, self.context_dist.get_weights(), dtype=torch.float64
        )
        ...
        kl_constraint = NonlinearConstraint(
            lambda x: self.old_kl_con(x, old_context_dist),
            -np.inf,
            self.max_kl,
        ...
        perf_constraint = NonlinearConstraint(
            lambda x: self.expected_performance(
                x, contexts_t, old_c_log_prob_t, c_val_t
            ),
            self.perf_lb,
            np.inf,
            jac=lambda x: self.expected_performance(
                x, contexts_t, old_c_log_prob_t, c_val_t, obj=False, grad=True
            ),
            keep_feasible=False,
        )

        if (
            self.kl_threshold is not None
            and self.target_context_kl() > self.kl_threshold
        ):
        ...
        try:
            ...
            if self.perf_lb_reached or perf_constraint.fun(x0) >= self.perf_lb:
                ...
                kl1_target_log_pdf_t = torch.from_numpy(
                    self.target_log_likelihood(kl1_samples_t.detach().numpy())
                )
                kl2_target_log_pdf_t = torch.from_numpy(
                    self.target_log_likelihood(kl2_samples_t.detach().numpy())
                )
                ...
                def objective(x):
                    ...
                    kl1_t = torch.mean(
                        torch.exp(kl1_new_log_pdf_t - kl1_log_pdf_t)
                        * (kl1_new_log_pdf_t - kl1_target_log_pdf_t)
                    )
                    ...
                    mu_grad, chol_flat_grad = torch.autograd.grad(
                        kl_t, dist.parameters()
                    )
                    return kl_t.detach().numpy() + 0.5, np.concatenate(
                        [mu_grad.detach().numpy(), chol_flat_grad.detach().numpy()]
                    ).astype(np.float64)
            ...
            # even once
                ...
                # Define the objective plus Jacobian
                objective = lambda x: self.expected_performance(
                    x, contexts_t, old_c_log_prob_t, c_val_t, grad=True, negate=True
                )
                res = minimize(
                    objective,
                    x0,
            ...
            with open(
                os.path.join("opt_errors", "error_" + str(time.time())), "wb"
            ) as f:
                pickle.dump((self.context_dist.get_weights(), contexts, values), f)
            print(
                "Exception occurred during optimization! Storing state and re-raising!"
            )
            raise e
        ...
        if self.perf_lb_reached:
            ...
            if old_perf < 0.95 * self.perf_lb and new_perf < 0.95 * self.perf_lb:
                print(
                    "Could not recover from performance loss in optimization. Will keep old distribution"
                )
                perf_ok = False
            else:
                perf_ok = True
        ...
        if perf_ok and std_ok and res.fun <= old_f:
            ...
            print(
                "Warning! Context optimihation unsuccessful - will keep old values. Message: %s"
                % res.message
            )

        if self.callback is not None:
            self.callback(
                old_context_dist.mean(),
                old_context_dist.covariance_matrix(),
                self.context_dist.mean(),
                self.context_dist.covariance_matrix(),
                contexts,
                values,
            )
    ...
    def sample(self):
        ...
        while not sample_ok and count < 100:
            ...
            sample_ok = np.all(self.context_bounds[0] <= sample) and (
                np.all(sample <= self.context_bounds[1])
            )
            count += 1
        ...
        if sample_ok:
            ...
            if np.all(self.context_bounds[0] <= mu) and (
                np.all(mu <= self.context_bounds[1])
            ):
