...
from deep_sprl.teachers.spl.wasserstein_interpolation import (
    SamplingWassersteinInterpolation,
)
...
class CurrOT(AbstractTeacher):
    ...
    def __init__(
        ...
        self.teacher = SamplingWassersteinInterpolation(
            init_samples,
            target_sampler,
            perf_lb,
            np.sqrt(epsilon),
        ...
        self.success_buffer = WassersteinSuccessBuffer(
            perf_lb, init_samples.shape[0], epsilon, context_bounds=context_bounds
        )
    ...
    def update_distribution(self, contexts, returns):
        t_up1 = time.time()
        fail_contexts, fail_returns = self.success_buffer.update(
            contexts,
            returns,
            self.teacher.target_sampler(self.teacher.current_samples.shape[0]),
        )

        if self.threshold_reached:
            self.fail_context_buffer.extend(fail_contexts)
            self.fail_context_buffer = self.fail_context_buffer[
                -self.teacher.n_samples :
            ]
        ...
        if len(self.fail_context_buffer) == 0:
            ...
            train_contexts = np.concatenate(
                (np.stack(self.fail_context_buffer, axis=0), success_contexts), axis=0
            )
            train_returns = np.concatenate(
                (np.stack(self.fail_return_buffer, axis=0), success_returns), axis=0
            )
        model = NadarayaWatson(
            train_contexts, train_returns, 0.3 * self.teacher.epsilon
        )
        t_up2 = time.time()
        ...
        if self.threshold_reached or avg_perf >= self.teacher.perf_lb:
            ...
            print(
                "Current performance: %.3e vs %.3e" % (avg_perf, self.teacher.perf_lb)
            )
            if self.wait_until_threshold:
                print(
                    "Not updating sampling distribution, as performance threshold not met"
                )
            else:
                # Which update is better is better?
                self.teacher.update_distribution(
                    model, self.success_buffer.read_update()
                )
                # self.teacher.current_samples = self.success_buffer.read_update()
        t_mo2 = time.time()

        # print(
        #     "Total update took: %.3e (Buffer/Update: %.3e/%.3e)"
        #     % (t_mo2 - t_up1, t_up2 - t_up1, t_mo2 - t_mo1)
        # )
...
class AbstractSuccessBuffer(ABC):
    ...
    @abstractmethod
    def update_delta_not_reached(
        self,
        new_contexts: np.ndarray,
        new_returns: np.ndarray,
        current_samples: np.ndarray,
    ) -> Tuple[bool, np.ndarray, np.ndarray, List[bool]]:
        pass

    @abstractmethod
    def update_delta_reached(
        self,
        new_contexts: np.ndarray,
        new_returns: np.ndarray,
        current_samples: np.ndarray,
    ...
    def update(self, contexts, returns, current_samples):
        ...
        if not self.delta_reached:
            self.delta_reached, self.contexts, self.returns, mask = (
                self.update_delta_not_reached(contexts, returns, current_samples)
            )
        else:
            self.contexts, self.returns, mask = self.update_delta_reached(
                contexts, returns, current_samples
            )
    ...
    def read_update(self):
        ...
        var_scales = np.clip(self.delta - self.returns, 0.0, np.inf) / (
            self.delta - self.min_ret
        )
        ...
        if not self.delta_reached:
            ...
            sampled_contexts = np.concatenate(
                (self.contexts, self.contexts[add_idxs, :]), axis=0
            )
            sampled_stds = np.concatenate((stds, stds[add_idxs, :]), axis=0)

        contexts = sampled_contexts + np.random.normal(
            0, sampled_stds, size=(self.max_size, self.contexts.shape[1])
        )
        invalid = np.any(
            np.logical_or(
                contexts < self.context_bounds[0][None, :],
                contexts > self.context_bounds[1][None, :],
            ),
            axis=-1,
        )
        count = 0
        while np.any(invalid) and count < 10:
            new_noise = np.random.normal(
                0,
                sampled_stds[invalid, :],
                size=(np.sum(invalid), self.contexts.shape[1]),
            )
...
class WassersteinSuccessBuffer(AbstractSuccessBuffer):
    ...
    def update_delta_not_reached(
        self, contexts: np.ndarray, returns: np.ndarray, current_samples: np.ndarray
    ) -> Tuple[bool, np.ndarray, np.ndarray, List[bool]]:
        ...
        new_contexts = np.concatenate(
            (contexts[mask, :], self.contexts[offset_idx:, :]), axis=0
        )
        ...
        return (
            new_delta_reached,
            new_contexts[sort_idxs, :],
            new_returns[sort_idxs],
            rem_mask,
        )
        ...
        if n_new > 0:
            ...
            if not np.any(remove_mask) and self.returns.shape[0] >= self.max_size:
                extended_contexts = np.concatenate(
                    (self.contexts, contexts[mask, :]), axis=0
                )
                ...
                dists = np.sum(
                    np.square(
                        extended_contexts[:, None, :] - current_samples[None, :, :]
                    ),
                    axis=-1,
                )
                ...
                mask[mask] = [
                    idx in (ret_idxs - self.contexts.shape[0])
                    for idx in np.arange(n_new)
                ]

                avg_dist = sliced_wasserstein(new_contexts, current_samples)
                # print(
                #     "Updated success buffer with %d samples. New Wasserstein distance: %.3e"
                #     % (n_new, avg_dist)
                # )
            else:
                ...
                new_returns = np.concatenate(
                    (returns[mask], self.returns[~remove_mask]), axis=0
                )
                new_contexts = np.concatenate(
                    (contexts[mask, :], self.contexts[~remove_mask, :]), axis=0
                )
