...
class FCGAN(object):
    def __init__(
        ...
        self.generator = Generator(
            generator_output_size, generator_layers, noise_size, self.configs
        )
        ...
        self.generator.apply(
            partial(
                weights_init,
                self.configs["generator_weight_initializer"],
                torch.nn.init.zeros_,
            )
        )
        self.discriminator.apply(
            partial(
                weights_init,
                self.configs["discriminator_weight_initializer"],
                torch.nn.init.zeros_,
            )
        )

        self.generator_optimizer = self.configs["generator_optimizer"](
            self.generator.parameters(), lr=1e-3
        )
        self.discriminator_optimizer = self.configs["discriminator_optimizer"](
    ...
    def train_generator(self, X, iters):
        ...
        for i in range(iters):
            ...
            loss = torch.mean(
                torch.square(self.discriminator.forward(generated_samples) - 1)
            )
    ...
    def discriminator_predict(self, X):
        ...
        for i in range(0, X.shape[0], batch_size):
            ...
            output.append(
                self.discriminator.forward(torch.from_numpy(X[i : i + sample_size]))
                .detach()
                .numpy()
            )
