...
class BaseWrapper(gym.Env):
    ...
    def __init__(
        ...
        if context_visible:
            ...
            low_ext = np.concatenate(
                (self.env.observation_space.low, -np.inf * np.ones_like(context))
            )
            high_ext = np.concatenate(
                (self.env.observation_space.high, np.inf * np.ones_like(context))
            )
...
class BaseVecEnvWrapper(SubprocVecEnv):
    ...
    def __init__(
        ...
        if context_visible:
            ...
            low_ext = np.concatenate(
                (self.observation_space.low, -np.inf * np.ones_like(context))
            )
            high_ext = np.concatenate(
                (self.observation_space.high, np.inf * np.ones_like(context))
            )
            self.observation_space = gym.spaces.Box(low=low_ext, high=high_ext)
        else:
    ...
    def reset_async(self, env_id, force=False):
        ...
        if self.context_post_processing is None:
            ...
            self.processed_contexts[env_id] = self.context_post_processing(
                self.cur_contexts[env_id].copy()
            )
    ...
    def step_wait(self):
        ...
        if self.context_visible:
            obs = [
                np.concatenate([ob, self.processed_contexts[i]], axis=0)
        ...
        if self.ret_rms is None:
            ...
            normalized_rewards = np.clip(
                np.stack(rewards) / np.sqrt(self.ret_rms.var + 1e-4), -10.0, 10.0
            )
        return (
            _flatten_obs(new_obs, self.observation_space),
            normalized_rewards,
            np.stack(dones),
            infos,
        )
    ...
    def reset_data(self, env_id):
        self.undiscounted_rewards[env_id] = 0.0
        self.discounted_rewards[env_id] = 0.0
        self.cur_discs[env_id] = 1.0
        self.step_lengths[env_id] = 0.0
    ...
    def update(self, step):
        ...
        for i in range(0, len(self.processes)):
            if step[2][i]:
                ...
                self.done_callback(
                    cur_step,
                    self.cur_initial_states[i].copy(),
                    self.cur_contexts[i].copy(),
                    self.discounted_rewards[i],
                    self.undiscounted_rewards[i],
                )
