...
from deep_sprl.teachers.spl import SelfPacedTeacher
...
class CurriculumType(Enum):
    ...
    def cp_drl(self):
        return self.value == CurriculumType.CPDRL.value
    ...
    def from_string(string):
        ...
        elif string == str(CurriculumType.CPDRL):
            return CurriculumType.CPDRL
        else:
...
class PPOInterface(AgentInterface):
    ...
    def estimate_value_internal(self, inputs):
        return np.squeeze(
            self.learner.policy.predict_values(torch.from_numpy(inputs))
            .detach()
            .numpy()
        )
    ...
    def mean_policy_std(self, cb_args, cb_kwargs):
        std_th = self.learner.policy.get_distribution(
...
class Learner(Enum):
    ...
    def create_learner(self, env, parameters):
        ...
        if self.ppo():
            model = PPO(
                PPOMlpPolicy, env, **parameters["common"], **parameters[str(self)]
            )
...
class ExperimentCallback:
    ...
    def __init__(
        self, log_directory, learner, env_wrapper, save_interval=5, step_divider=1
    ):
        ...
        if self.env_wrapper.teacher is not None:
            if isinstance(self.env_wrapper.teacher, SelfPacedTeacher):
    ...
    def __call__(self, *args, **kwargs):
        if self.algorithm_iteration % self.step_divider == 0:
            ...
            if isinstance(self.env_wrapper.teacher, SelfPacedTeacher):
                ...
                context_std = np.sqrt(
                    np.diag(self.env_wrapper.teacher.context_dist.covariance_matrix())
                )
...
class AbstractExperiment(ABC):
    APPENDIX_KEYS = {
        ...
        CurriculumType.CPDRL: ["DELTA", "KL_EPS"],
