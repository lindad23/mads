...
from deep_sprl.teachers.spl import SelfPacedTeacher, SelfPacedWrapper, CurrOT
from deep_sprl.teachers.cp_drl import CPDRLWrapper
...
class MazeSampler:
    def __init__(self):
        self.LOWER_CONTEXT_BOUNDS = np.array([-9.0, -9.0, 0.05])
        self.UPPER_CONTEXT_BOUNDS = np.array([9.0, 9.0, 0.05])
...
class MazeExperiment(AbstractExperiment):
    INITIAL_MEAN = np.array([0.0, 0.0, 10])
    INITIAL_VARIANCE = np.diag(np.square([4, 4, 5]))

    TARGET_LOWER_CONTEXT_BOUNDS = np.array([-9.0, -9.0, 0.05])
    TARGET_UPPER_CONTEXT_BOUNDS = np.array([9.0, 9.0, 0.05])

    LOWER_CONTEXT_BOUNDS = np.array([-9.0, -9.0, 0.05])
    UPPER_CONTEXT_BOUNDS = np.array([9.0, 9.0, 18.0])
    ...
    def target_log_likelihood(self, cs):
        norm = np.prod(
            self.UPPER_CONTEXT_BOUNDS[:2] - self.LOWER_CONTEXT_BOUNDS[:2]
        ) * (0.01 * 1 + 17.94 * 1e-4)
        return np.where(
            cs[:, -1] < 0.06,
            np.log(1 / norm) * np.ones(cs.shape[0]),
    ...
    def target_sampler(self, n, rng=None):
        if rng is None:
            rng = np.random
        return rng.uniform(
            self.TARGET_LOWER_CONTEXT_BOUNDS,
    ...
    def create_environment(self, evaluation=False):
        ...
        if evaluation or self.curriculum.default():
            ...
            init_samples = np.random.uniform(
                self.LOWER_CONTEXT_BOUNDS, self.UPPER_CONTEXT_BOUNDS, size=(1000, 3)
            )
            teacher = GoalGAN(
                self.LOWER_CONTEXT_BOUNDS.copy(),
                self.UPPER_CONTEXT_BOUNDS.copy(),
            ...
            env = SelfPacedWrapper(
                ...
                episodes_per_update=self.EP_PER_UPDATE,
            )
        elif self.curriculum.cp_drl():
            teacher = self.create_self_paced_teacher(with_callback=False)
            env = CPDRLWrapper(
                env,
                teacher,
            ...
            bins = 20
            teacher = ACL(
                bins * bins * bins, self.ACL_ETA, eps=self.ACL_EPS, norm_hist_len=2000
            )
            env = ACLWrapper(
                env,
                teacher,
                self.DISCOUNT_FACTOR,
                context_visible=True,
                context_post_processing=Subsampler(
                    self.LOWER_CONTEXT_BOUNDS.copy(),
                    self.UPPER_CONTEXT_BOUNDS.copy(),
                    [bins, bins, bins],
                ),
            ...
            teacher = PLR(
                ...
                self.PLR_REPLAY_RATE,
                self.PLR_BUFFER_SIZE,
                self.PLR_BETA,
                self.PLR_RHO,
            )
            env = PLRWrapper(
                ...
                value_fn=ValueFunction(
                    env.observation_space.shape[0] + self.LOWER_CONTEXT_BOUNDS.shape[0],
                    [128, 128, 128],
                    torch.nn.ReLU(),
                    {
                        "steps_per_iter": 2048,
            ...
            teacher = VDS(
                ...
                self.VDS_NQ,
                q_train_config={
                    "replay_size": 5 * self.STEPS_PER_ITER,
                    "lr": self.VDS_LR,
                    "n_epochs": self.VDS_EPOCHS,
                    "batches_per_epoch": self.VDS_BATCHES,
    ...
    def create_learner_params(self):
        return dict(
            ...
            ppo=dict(
                n_steps=int(self.STEPS_PER_ITER), batch_size=320, gae_lambda=self.LAM
            ),
            sac=dict(
                learning_rate=3e-4,
                buffer_size=200000,
    ...
    def create_experiment(self):
        ...
        if isinstance(env, VDSWrapper):
            env.teacher.initialize_teacher(
                env,
                interface,
    ...
    def create_self_paced_teacher(self, with_callback=False):
        ...
        elif self.curriculum.wasserstein() or self.curriculum.cp_drl():
            init_samples = np.random.uniform(
                np.array([-9.0, -9.0, 18.0]), np.array([9.0, 9.0, 18.0]), size=(500, 3)
            )
            return CurrOT(
                bounds, init_samples, self.target_sampler, self.DELTA, self.METRIC_EPS
            )
        else:
