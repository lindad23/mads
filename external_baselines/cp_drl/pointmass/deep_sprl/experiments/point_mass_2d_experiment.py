...
class PointMass2DExperiment(AbstractExperiment):
    TARGET_MEANS = np.array([[3.0, 0.5], [-3.0, 0.5]])
    TARGET_VARIANCES = np.array([np.diag([1e-4, 1e-4]), np.diag([1e-4, 1e-4])])

    LOWER_CONTEXT_BOUNDS = np.array([-4.0, 0.5])
    UPPER_CONTEXT_BOUNDS = np.array([4.0, 8.0])

    def target_log_likelihood(self, cs):
        p0 = multivariate_normal.logpdf(
            cs, self.TARGET_MEANS[0], self.TARGET_VARIANCES[0]
        )
        p1 = multivariate_normal.logpdf(
            cs, self.TARGET_MEANS[1], self.TARGET_VARIANCES[1]
        )
    ...
    def target_sampler(self, n, rng=None):
        ...
        s0 = rng.multivariate_normal(
            self.TARGET_MEANS[0], self.TARGET_VARIANCES[0], size=n
        )
        s1 = rng.multivariate_normal(
            self.TARGET_MEANS[1], self.TARGET_VARIANCES[1], size=n
        )

        return decisions[:, None] * s0 + (1 - decisions)[:, None] * s1

    INITIAL_MEAN = np.array([0.0, 4.25])
    ...
    def create_environment(self, evaluation=False):
        ...
        if evaluation or self.curriculum.default():
            ...
            samples = np.random.uniform(
                self.LOWER_CONTEXT_BOUNDS, self.UPPER_CONTEXT_BOUNDS, size=(1000, 2)
            )
            teacher = GoalGAN(
                self.LOWER_CONTEXT_BOUNDS.copy(),
                self.UPPER_CONTEXT_BOUNDS.copy(),
            ...
            bins = 50
            teacher = ACL(
                bins * bins, self.ACL_ETA, eps=self.ACL_EPS, norm_hist_len=2000
            )
            env = ACLWrapper(
                env,
                teacher,
    ...
    def create_learner_params(self):
        return dict(
            common=dict(
                ...
                device="cpu",
                policy_kwargs=dict(
                    net_arch=[128, 128, 128], activation_fn=torch.nn.Tanh
                ),
            ),
    ...
    def create_self_paced_teacher(self, with_callback=False):
        ...
        if self.curriculum.self_paced():
            ...
            init_samples = np.random.uniform(
                self.LOWER_CONTEXT_BOUNDS, self.UPPER_CONTEXT_BOUNDS, size=(100, 2)
            )
            return CurrOT(
                bounds,
                init_samples,
