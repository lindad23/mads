...
class ValueFunction(torch.nn.Module):
    ...
    def forward(self, x):
        if isinstance(x, np.ndarray):
            return np.mean(
                self.net(torch.from_numpy(x).float()).detach().numpy().astype(x.dtype)
            )
        else:
            return self.net(x)
    ...
    def predict_individual(self, samples, with_gradient=False):
        ...
        if with_gradient:
            grad = torch.autograd.grad(
                perf, samples_torch, grad_outputs=torch.ones_like(perf)
            )[0]
            return np.squeeze(
                perf.detach().numpy().astype(samples.dtype)
...
def train_vf(vf, data_x, data_y, max_iter=10000):
    ...
    rel_err = torch.mean(
        torch.abs(data_y - torch.squeeze(vf.forward(data_x)))
        / torch.clamp_min(torch.abs(data_y), 1.0)
    )
    count = 0
...
def get_vf():
    ...
    train_y = np.minimum(1.0, 1 / (10 * train_x))
    ...
    for row, target_ll in enumerate(target_lls):
        ...
        for i, delta in enumerate(deltas):
            ...
            if expected_value(target_ll, values, 0) >= delta:
                ...
                eta = brentq(
                    lambda x: expected_value(target_ll, values, x) - delta, 2000.0, 0.0
                )
            interp_ll = target_ll + eta * values
            interp_ll -= logsumexp(interp_ll)
            ...
            (tl,) = axs[i].plot(
                xs, np.exp(target_ll) / np.exp(np.max(target_ll)), color="C0"
            )
            axs[i].fill_between(
                xs,
                0.0,
                np.exp(target_ll) / np.exp(np.max(target_ll)),
                color="C0",
                alpha=0.5,
            )
            (il,) = axs[i].plot(
                xs, np.exp(interp_ll) / np.exp(np.max(interp_ll)), color="C2"
            )
            axs[i].fill_between(
                xs,
                0.0,
                np.exp(interp_ll) / np.exp(np.max(interp_ll)),
                color="C2",
                alpha=0.5,
            )
            ...
            axs[i].fill_between(xs, 0.0, gls / np.max(gls), color="C5", alpha=0.5)

            # axs[i].legend([vl, tl, il], [r"$J(\pi, c)$", r"$\mu(c)$", r"$\mu(c)\exp(J(\pi, c))^{\eta}$"],
            #               fontsize=TICK_SIZE)
            if row == 1:
                axs[i].set_title(r"$\delta=%.2f$" % delta, fontsize=FONT_SIZE, pad=0.0)
...
def kl_interpolation_plot2(path=None):
    ...
    target_lls1 = []
    target_lls2 = []
    alphass = [[0.0, 0.45, 0.5, 0.55, 1.0], [0.0, 0.25, 0.5, 0.75, 1.0]]
    ...
    for row, (target_ll1, target_ll2, alphas) in enumerate(
        zip(target_lls1, target_lls2, alphass)
    ):
        # Generate the different plots for the different eta
        axs = [
            f.add_axes(
                [
                    0.83 * i / len(alphas) + 0.005,
                    0.1 + 0.42 * row,
                    0.83 / len(alphas) - 0.008,
                    0.4,
                ]
            )
        ...
        for i, alpha in enumerate(alphas):
            ...
            axs[i].fill_between(
                xs, 0, (linit - min_l) / (max_l - min_l), color="C0", alpha=0.5
            )
            (p2l,) = axs[i].plot(xs, (ltarget - min_l) / (max_l - min_l), color="C1")
            axs[i].fill_between(
                xs, 0, (ltarget - min_l) / (max_l - min_l), color="C1", alpha=0.5
            )
            (il,) = axs[i].plot(xs, (linterp - min_l) / (max_l - min_l), color="C2")
            axs[i].fill_between(
                xs, 0, (linterp - min_l) / (max_l - min_l), color="C2", alpha=0.5
            )
                ...
                if i == 0:
                    axs[i].legend(
                        ...
                        [
                            r"$p_1(c)$",
                            r"$p_2(c)$",
                            r"$p_1(c)^{\alpha} p_2(c)^{1-\alpha}$",
                        ],
                        ncol=1,
                        fontsize=FONT_SIZE,
                        loc="center right",
                        bbox_to_anchor=(6.3, 0.0),
                        handlelength=0.4,
                        columnspacing=0.0,
                        handletextpad=0.3,
                    )
...
def wasserstein_plot(path=None):
    ...
    target_ll_fns = [
        lambda x: np.log(np.where(x > 0.76, 1, 1e-5)),
    ...
    for row, (target_ll_fn, cache_dir) in enumerate(zip(target_ll_fns, cache_dirs)):
        ...
        target_ll -= logsumexp(target_ll)
        target_samples = generate_histogram_samples(
            target_ll_fn, np.linspace(0, 1, 51), 1000
        )
        ...
        for delta in deltas:
            ...
            if not os.path.exists(cache_path):
                init_samples = np.random.uniform(
                    0.0, np.max(xs[values >= delta]), n_samples
                )[:, None]
                curriculum = WassersteinInterpolation(
                    init_samples,
                    target_sampler,
                    perf_lb=delta,
                    epsilon=10.0,
        ...
        axs = [
            fig.add_axes(
                [
                    0.87 * i / len(deltas) + 0.005,
                    0.1 + 0.42 * row,
                    0.87 / len(deltas) - 0.01,
                    0.4,
                ]
            )
            for i in range(len(deltas))
        ...
        for i, delta in enumerate(deltas):
            interp, bin_edges = np.histogram(
                np.squeeze(samples[delta]), bins=50, range=(0, 1)
            )
            ...
            axs[i].fill_between(xs, 0.0, target / scale, color="C0", alpha=0.5)
            il = axs[i].bar(
                bin_edges[:-1] + 0.5 * (bin_edges[1:] - bin_edges[:-1]),
                interp / scale,
            ...
            axs[i].hlines(delta, 0.0, 1.0, color="black", linestyles="--")
            axs[i].fill_between(xs, 0.0, values, color="C1", alpha=0.3)
            if row == 1:
                if i == 0:
                    axs[i].legend(
                        [vl, tl, il],
                        [r"$J(\pi, c)$", r"$\mu(c)$", r"$p_{\mathcal{W}}(c)$"],
                        ncol=1,
                        fontsize=FONT_SIZE,
                        loc="center right",
                        bbox_to_anchor=(5.9, 0.0),
                        handlelength=0.4,
                        columnspacing=0.0,
                        handletextpad=0.3,
                    )
