...
def main():
    ...
    parser.add_argument(
        "--env", type=str, default="maze", choices=["point_mass_2d", "maze"]
    )
    ...
    for method, params in alg_params.items():
        top_hps = select_best_hps(
            exp, args.learner, method, {k: v for (k, v) in params}, args.base_log_dir
        )
        param_comp(
            exp,
            args.learner,
