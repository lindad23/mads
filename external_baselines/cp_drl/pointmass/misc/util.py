...
def add_plot(base_log_dir, ax, color, max_seed=None, marker="o", markevery=3):
    ...
    seeds = [
        int(d.split("-")[1]) for d in os.listdir(base_log_dir) if d.startswith("seed-")
    ]
    for seed in [s for s in seeds if s <= max_seed]:
        ...
        if os.path.exists(os.path.join(seed_log_dir, "performance.pkl")):
            iteration_dirs = [
                d for d in os.listdir(seed_log_dir) if d.startswith("iteration-")
            ]
    ...
    if len(seed_performances) > 0:
        ...
        min_length = np.min(
            [len(seed_performance) for seed_performance in seed_performances]
        )
        iterations = iterations[0:min_length]
        seed_performances = [
            seed_performance[0:min_length] for seed_performance in seed_performances
        ]
...
def param_comp(
    ...
    plt.xlabel("Epoch", fontsize=font_size, labelpad=2.0)
    f.legend(
        lines,
        labels,
        fontsize=font_size,
