...
def performance_plot(ax, base_log_dir="logs"):
    ...
    for method, color in zip(
        ["self_paced", "random", "default", "cp_drl", "wasserstein"],
        ["C0", "C2", "C3", "C4", "C1"],
    ):
        exp = PointMass2DExperiment(base_log_dir, method, "ppo", {}, seed=1)
    ...
    for method, color in zip(
        ["goal_gan", "alp_gmm", "acl", "plr", "vds"], ["C5", "C6", "C7", "C8", "C9"]
    ):
        exp = PointMass2DExperiment(base_log_dir, method, "ppo", {}, seed=1)
        log_dir = os.path.dirname(exp.get_log_dir())
        lines.append(add_plot(log_dir, ax, color))
...
def sprl_plot(
    seed, iterations, path=None, axs=None, colors=None, s=10, base_log_dir="logs"
):
...
def currot_plot(
    seed, iterations, axs=None, path=None, colors=None, s=10, base_log_dir="logs"
):
    exp = PointMass2DExperiment(base_log_dir, "wasserstein", "ppo", {}, seed=1)
    log_dir = os.path.dirname(exp.get_log_dir())
    ...
    if axs is None:
        ...
        axs = [
            f.add_axes(
                [0.5 * (i % 2) + 0.005, 0.5 * (1 - (i // 2)) + 0.005, 0.48, 0.48]
            )
            for i in range(4)
        ]
    ...
    for i, iteration in enumerate(iterations):
        ...
        ax.scatter(
            teacher.teacher.current_samples[:, 0],
            teacher.teacher.current_samples[:, 1],
            s=s,
            alpha=0.2,
            color=colors[i],
        )
        ...
        for seed_dir in seed_dirs:
            ...
            for iteration in iterations:
                ...
                if len(trace[0]) != 0:
                    ...
                    if dim == 0:
                        min_width_dist = np.minimum(
                            np.abs(contexts[:, 0] - (-3)), np.abs(contexts[:, 0] - 3)
                        )
                    else:
            ...
            if len(avg_dists) < len(iterations):
                avg_dists = [avg_dists[0]] * (
                    len(iterations) - len(avg_dists)
                ) + avg_dists
...
def precision_comparison(axs=None, path=None, base_log_dir="logs"):
    ...
    for i in range(0, 2):
        ...
        axs[i].set_ylabel(
            "Gate Position" if i == 0 else "Gate Width", fontsize=FONT_SIZE, labelpad=1
        )
...
if __name__ == "__main__":
    ...
    combined_plot(
        path="figures/point_mass_precision+performance.pdf", base_log_dir=base_log_dir
    )
