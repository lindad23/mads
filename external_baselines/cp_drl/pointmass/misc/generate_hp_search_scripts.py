...
alg_params = {
    "alp_gmm": [
        ("AG_P_RAND", [0.1, 0.2, 0.3]),
        ("AG_FIT_RATE", [50, 100, 200]),
        ("AG_MAX_SIZE", [500, 1000, 2000]),
    ],
    "goal_gan": [
        ("GG_NOISE_LEVEL", [0.025, 0.05, 0.1]),
        ("GG_FIT_RATE", [50, 100, 200]),
        ("GG_P_OLD", [0.1, 0.2, 0.3]),
    ],
    "acl": [("ACL_EPS", [0.05, 0.1, 0.2]), ("ACL_ETA", [0.01, 0.025, 0.05])],
    "plr": [
        ("PLR_REPLAY_RATE", [0.55, 0.7, 0.85]),
        ("PLR_BETA", [0.15, 0.3, 0.45]),
        ("PLR_RHO", [0.15, 0.3, 0.45]),
    ],
    "vds": [
        ("VDS_LR", [0.0001, 0.0005, 0.001]),
        ("VDS_EPOCHS", [3, 5, 10]),
        ("VDS_BATCHES", [20, 40, 80]),
    ],
}
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="alp_gmm",
        choices=["alp_gmm", "goal_gan", "acl", "plr", "vds", "cp_drl"],
    )
    ...
    for param_name, param_values in params:
        ...
        for cur_arg in extra_args:
            for param_value in param_values:
                new_extra_args.append(
                    cur_arg + "--{0} {1} ".format(param_name, param_value)
                )
    ...
    for i, cur_extra_args in enumerate(extra_args):
        ...
        script_dir = os.path.join(
            base_dir, "cluster_scripts", "hp_search", args.type, args.env, args.learner
        )
