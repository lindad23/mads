...
def plot_trajectories(seed, path=None, base_log_dir="logs"):
    ...
    for cur_type in types:
        f = plt.figure(figsize=(1.0, 1.0))
        ax = plt.Axes(f, [0.0, 0.0, 1.0, 1.0])
        f.add_axes(ax)

        width = 0.35
        ax.plot([-5.0, -3 - width], [-0.1, -0.1], linewidth=3, color="black")
        ax.plot([-3 + width, 3 - width], [-0.1, -0.1], linewidth=3, color="black")
        ax.plot([3 + width, 5.0], [-0.1, -0.1], linewidth=3, color="black")
        ...
        iter_path = os.path.join(
            ...
            "..",
            exp.get_log_dir(),
            "iteration-%d" % eval_iter,
        )

        if os.path.exists(iter_path):
            model = exp.learner.load_for_evaluation(
                os.path.join(iter_path, "model"), exp.vec_eval_env
            )
...
def draw_env(ax, pos, width):
    ax.plot([-5.0, pos - (0.5 * width + 0.5)], [-0.1, -0.1], linewidth=3, color="black")
    ax.plot([pos + (0.5 + 0.5 * width), 5.0], [-0.1, -0.1], linewidth=3, color="black")

    ax.scatter([0.0], [3.0], s=10, color="black")
...
def context_space_image(path=None, sprl=False, base_log_dir="logs"):
    ...
    ax.set_yticks(np.arange(0.0, 8), minor=True)
    ...
    if sprl:
        sprl_plot(
            ...
            [10, 25, 35, 50],
            axs=[ax, ax, ax, ax],
            colors=[colormap(0.1), colormap(0.3), colormap(0.5), colormap(1.0)],
            s=30,
...
if __name__ == "__main__":
    ...
    context_space_image(
        "figures/point_mass_env.pdf", sprl=False, base_log_dir=base_log_dir
    )
    context_space_image(
        "figures/point_mass_distribution_sprl.pdf", sprl=True, base_log_dir=base_log_dir
    )
    plot_trajectories(
        1, path="figures/point_mass_trajectories", base_log_dir=base_log_dir
    )
