...
t1 = time.time()
for i in range(10):
    res = smoother.predict(test_data, bandwidth=1.0, n_max=300)
t2 = time.time()
...
z = smoother.predict(
    np.reshape(np.stack((x, y), axis=-1), (-1, 2)), bandwidth=0.5, n_max=50
).reshape(x.shape)

f, axs = plt.subplots(1, 2)
im = axs[0].imshow(
    z_gt, cmap=cm.RdBu, extent=[-np.pi, np.pi, -np.pi, np.pi], origin="lower"
)  # drawing the function
cset = axs[0].contour(
    x, y, z_gt, linewidths=2, cmap=cm.Set2, vmin=np.min(z_gt), vmax=np.max(z_gt)
)

im = axs[1].imshow(
    z, cmap=cm.RdBu, extent=[-np.pi, np.pi, -np.pi, np.pi], origin="lower"
)  # drawing the function
cset = axs[1].contour(
    x, y, z, linewidths=2, cmap=cm.Set2, vmin=np.min(z_gt), vmax=np.max(z_gt)
)
